...
\begin{columns}[T]
  ...
  \begin{itemize}\setlength{\itemsep}{3pt}
    {\scriptsize
    \item \important{阶段1}~~图像条件预热：仅训 LoRA，建立换脸生成能力
    \item \important{阶段2}~~混合过渡：$e_{id}{=}(1{-}\lambda)E_{id}(x_{real}){+}\lambda\mathcal{E}_\psi(y)$
    \item \important{阶段3}~~纯标签适配：启用分类器引导，优化攻击有效性
    }
  \end{itemize}

  \column{0.46\textwidth}
  \centering\footnotesize\textbf{分类器引导推理}\\[2pt]
  \includegraphics[width=\linewidth,height=0.34\textheight,keepaspectratio]{figures/infer_mia-1.pdf}\\[6pt]
  \footnotesize\textbf{梯度引导修正公式：}\\[2pt]
  {\scriptsize\begin{equation*}
...
\begin{columns}[c]
  ...
  \vfill
  \centering
  \includegraphics[width=\linewidth,height=0.46\textheight,keepaspectratio]{figures/mia_training_curves.pdf}\\[1pt]
  {\scriptsize 训练曲线：FID持续下降、KNN与攻击准确率协同提升\\不确定性参数 $\sigma_i$ 动态调整，实现多目标梯度平衡}
  \vfill
\end{columns}
\end{frame} % 消融实验分析
