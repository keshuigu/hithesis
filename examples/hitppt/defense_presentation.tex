...
\subtitle{硕士学位论文答辩}
\author[俞磊]{答辩人：俞磊 \\ \smallskip \small 指导教师：王莘 教授}
...
\begin{itemize}\setlength{\itemsep}{0pt}
  \item \highlight{EDM生成骨干：}相比标准DDPM，EDM采用连续时间ODE公式与输入/输出预条件化，以更少采样步数生成更高保真人脸，且确定性ODE采样器使模板梯度引导更精确、更稳定
...
\end{equation*}}
{\scriptsize 引导强度 $\alpha$ 权衡生成质量与特征匹配度}
\end{column}
\end{columns}
\end{frame} % 训练策略与推理流程
...
\begin{column}{0.5\textwidth}
\textbf{攻击威胁模型}\\[2pt]
分类器 $F_\theta\!:\!\mathbb{R}^{HWC}\!\to\!\mathbb{R}^N$ 输出类别概率分布。攻击者仅给定类别标签 $y$，目标为生成高保真攻击样本 $\hat{x}$：
\vspace{-4pt}
{\scriptsize\begin{equation*}
\hat{x}=\arg\max_x\;F_\theta(x)_y+\lambda_{perc}\mathcal{L}_{perc}(x)-\lambda_{reg}R(x),\;\hat{x}\!\in\!\mathcal{M}_{nat}
\end{equation*}}\vspace{-4pt}
\textbf{攻击成功判据}
\begin{itemize}\setlength{\itemsep}{1pt}
  \item \important{强成功：}$F_\theta(\hat{x})_y\geq\tau$，高置信通过目标分类器
  \item \important{弱成功：}目标类别输出显著高于其余类，部分恢复身份特征
  \item \important{视觉成功：}自然人脸外观，通过人类视觉检验
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
\textbf{核心挑战}
\begin{itemize}\setlength{\itemsep}{2pt}
  \item \important{逆问题困难：}标签空间 $\{1,\ldots,N\}$ 到图像空间 $\mathbb{R}^{HWC}$ 缺乏直接映射，同一类别对应无穷多可行解
  \item \important{多目标权衡：}分类器置信度优化目标与视觉保真度目标在梯度方向上相互竞争
  \item \important{样本多样性：}对同一标签需生成覆盖不同姿态、表情、光照的多样化攻击集合
\end{itemize}
\end{column}
\end{columns}
\end{frame} % MIA问题定义与核心挑战

\begin{frame}{MIA核心设计要点}
\footnotesize
\begin{columns}[T]
\begin{column}{0.50\textwidth}
\textbf{为何选用换脸先验（REFace）？}
\begin{itemize}\setlength{\itemsep}{2pt}
  \item \highlight{身份-属性显式解耦：}身份嵌入经交叉注意力注入去噪过程，独立于姿态/表情，天然支持多样化生成
  \item \highlight{丰富面部结构先验：}在大规模人脸数据集预训练，细节保真度远优于从零训练的类条件生成模型
  \item \highlight{端到端可微：}全链路支持分类器梯度反向传播，可直接优化攻击目标
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{三层次改造方案}
\begin{itemize}\setlength{\itemsep}{2pt}
  \item \highlight{标签条件嵌入层 $\mathcal{E}_\psi$：}MLP 将独热编码 $y$ 映射为归一化身份嵌入 $e_{id}\!\in\!\mathbb{R}^{512}$，无缝替代图像编码器
  \item \highlight{LoRA 参数高效微调：}仅训练主干 1\%--5\% 参数（$r{=}16,\,\alpha{=}32$），冻结预训练权重同时适配新分布
  \item \highlight{渐进式三阶段训练：}图像预热 $\to$ 余弦退火混合过渡 $\to$ 纯标签适配，平滑实现模态迁移
\end{itemize}
\end{column}
\end{columns}
\vspace{4pt}
\begin{alertblock}{换脸先验迁移的核心障碍}
\scriptsize
标签嵌入输出分布与ArcFace 真实身份嵌入分布存在\textbf{显著偏移}，三层次改造方案正是针对此障碍而设计。
\end{alertblock}
\end{frame} % MIA核心设计要点

\begin{frame}{基于换脸先验迁移的多目标自适应模型反演方法整体架构}
\begin{columns}[c]
  ...
  \footnotesize\textbf{核心设计}
  \begin{itemize}\setlength{\itemsep}{0pt}
    \item \highlight{嵌入层 $\mathcal{E}_\psi$：}将离散标签映射为连续身份嵌入，替代原图像编码器
    \item \highlight{U-Net + LoRA：}冻结主干，低秩增量适配标签嵌入分布
    \item \highlight{ArcFace $E_{\text{id}}$：}监督身份一致性 $\mathcal{L}_{\text{id}}$，对齐超球面特征空间
    \item \highlight{分类器 $F_\theta$：}提供攻击引导 $\mathcal{L}_{\text{cls}}$ 与特征正则 $\mathcal{L}_{\text{p-reg}}$
  \end{itemize}
\end{columns}
\end{frame} % 方法整体架构
...
\begin{columns}[T]
  ...
  \begin{tabular}{@{}lp{0.72\linewidth}@{}}
    $\mathcal{L}_{\text{prior}}$ & 扩散去噪先验，保证生成样本位于自然人脸流形 \\
    $\mathcal{L}_{\text{cls}}$ & 分类引导（top-k margin），直接驱动攻击成功率 \\
    $\mathcal{L}_{\text{id}}$ & 身份一致性对比，精确对齐 ArcFace 超球面特征 \\
    $\mathcal{L}_{\text{perc}}$ & LPIPS 感知质量，确保视觉层面的真实感 \\
    $\mathcal{L}_{\text{p-reg}}$ & 特征正则，约束嵌入与真实分布的偏移 \\
    $\mathcal{L}_{\text{reg}}$ & 嵌入/LoRA 参数正则，防止过拟合 \\
  \end{tabular}}

  \column{0.44\textwidth}
  \begin{block}{任务不确定性自适应加权}
  \footnotesize
  \[
  \mathcal{L}_{\text{total}}=\sum_i\!\left(\frac{1}{2\sigma_i^2}\mathcal{L}_i+\frac{1}{2}\log\sigma_i^2\right)
  \]
  \end{block}
  \vspace{6pt}
  \begin{itemize}\setlength{\itemsep}{4pt}
    \scriptsize
    \item $\sigma_i$ 与模型参数\textbf{联合优化}，无需手动设定各损失权重
    \item 难优化任务 $\sigma_i\!\uparrow$ 自动降权，易收敛任务保持高权重
    \item 攻击有效性 $\mathcal{L}_{\text{cls}}$ 与生成保真度 $\mathcal{L}_{\text{prior}},\mathcal{L}_{\text{perc}}$ 动态协同平衡
  \end{itemize}
\end{columns}
\end{frame} % 多目标损失设计
...
\begin{columns}[T]
  ...
  \centering\footnotesize\textbf{三阶段渐进训练流程}\\[2pt]
  \includegraphics[width=\linewidth,height=0.34\textheight,keepaspectratio]{figures/train_mia.pdf}\\[6pt]
  \begin{itemize}\setlength{\itemsep}{3pt}
    {\scriptsize
    \item \important{阶段1}~~图像条件预热：冻结 $\mathcal{E}_\psi$，仅训 LoRA，以真实图像嵌入 $E_{id}(x_{real})$ 为条件，损失仅含 $\mathcal{L}_{prior}{+}\mathcal{L}_{perc}$，建立换脸生成能力
    \item \important{阶段2}~~混合条件过渡：解冻 $\mathcal{E}_\psi$，插值 $e_{id}{=}(1{-}\lambda(t))E_{id}(x_{real}){+}\lambda(t)\mathcal{E}_\psi(y)$（余弦退火），攻击损失随 $\lambda(t)$ 逐步激活
    \item \important{阶段3}~~纯标签条件适配：完全移除真实图像，$e_{id}{=}\mathcal{E}_\psi(y)$，解冻全部 LoRA 参数，启用完整 $\mathcal{L}_{total}$
    }
  \end{itemize}

  \column{0.46\textwidth}
  \centering\footnotesize\textbf{分类器引导推理}\\[2pt]
  \includegraphics[width=\linewidth,height=0.32\textheight,keepaspectratio]{figures/infer_mia-1.pdf}\\[4pt]
  \begin{itemize}\setlength{\itemsep}{3pt}
    {\scriptsize
    \item 每步去噪时注入分类器梯度，驱动采样轨迹持续逼近目标类别高置信区域
    }
  \end{itemize}
  \vspace{2pt}
  {\scriptsize\begin{equation*}
    \tilde{\epsilon}_\theta = \epsilon_\theta - \gamma(t)\sqrt{1{-}\bar{\alpha}_t}\,\nabla_{z_t}\log p_\theta(y|x_{0|t})
  \end{equation*}}
\end{columns}
\end{frame} % 训练与推理策略

\begin{frame}{实验结果与可视化}
\footnotesize
\begin{columns}[T]
  ...
  \begin{center}
  \textbf{跨架构泛化（vs. Diff-MI）}\\[1pt]
  {\tiny\renewcommand{\arraystretch}{0.9}
  \begin{tabular}{lccc}
  \toprule
  ...
  \end{tabular}}
  \vspace{4pt}
  \includegraphics[width=0.92\linewidth,height=3.2cm,keepaspectratio]{figures/grid_by_target.jpg}
  \end{center}
\end{columns}
\end{frame} % MIA实验结果
...
\begin{columns}[c]
  \column{0.52\textwidth}
  \footnotesize\textbf{训练策略消融}\\[0pt]
  \resizebox{\linewidth}{!}{\begin{tabular}{lcccc}
  \toprule
  \textbf{训练策略} & \textbf{TarAcc↑} & \textbf{EvalAcc↑} & \textbf{FID↓} & \textbf{KNN↑} \\
  \midrule
  单阶段（标签）  & 70.85\% & 63.74\% & 43.21 & 0.469 \\
  两阶段训练      & 88.92\% & 78.56\% & 32.45 & 0.625 \\
  \textbf{三阶段训练} & \textbf{94.87\%} & \textbf{83.15\%} & \textbf{23.26} & \textbf{0.717} \\
  \bottomrule
  \end{tabular}}
  \footnotesize\textbf{损失组合消融}\\[0pt]
  \resizebox{\linewidth}{!}{\begin{tabular}{lcccc}
  \toprule
  \textbf{损失组合} & \textbf{TarAcc↑} & \textbf{EvalAcc↑} & \textbf{FID↓} & \textbf{KNN↑} \\
  \midrule
  $\mathcal{L}_{\text{prior}}$（仅扩散先验）      &  7.85\% &  5.92\% & 53.47 & 0.416 \\
  $+\mathcal{L}_{\text{cls}}$（分类引导）         & 77.24\% & 70.38\% & 36.82 & 0.539 \\
  $+\mathcal{L}_{\text{id}}$（身份一致性）        & 92.56\% & 79.47\% & 29.15 & 0.674 \\
  $+\mathcal{L}_{\text{perc}}$（感知质量·完整）   & \textbf{94.87\%} & \textbf{83.15\%} & \textbf{23.26} & \textbf{0.717} \\
  \bottomrule
  \end{tabular}}
  \begin{exampleblock}{关键结论}\scriptsize
  \begin{itemize}\setlength{\itemsep}{0pt}
    \item \textbf{三阶段训练}相比单阶段：TarAcc \textbf{+24.02pp}，FID \textbf{↓19.95}
    \item $\mathcal{L}_{\text{cls}}$ 贡献最大（TarAcc +69.39pp），奠定攻击有效性基础
    \item $\mathcal{L}_{\text{id}}$ 显著提升保真度（EvalAcc +9.09pp，FID ↓7.67）
  \end{itemize}
  \end{exampleblock}
  ...
  \vfill
  \centering
  \includegraphics[width=\linewidth,height=0.42\textheight,keepaspectratio]{figures/mia_training_curves.pdf}\\[3pt]
  \begin{itemize}\setlength{\itemsep}{1pt}
    {\scriptsize
    \item FID 随生成质量优化持续收敛
    \item TarAcc 与 KNN 从阶段2起协同提升
    \item $\sigma_{\text{cls}}$ 等参数早期较大，随收敛自动下调，\\防止攻击目标过早主导训练
    }
  \end{itemize}
  \vfill
\end{columns}
...
\begin{itemize}
  ...
  \item MOBIO攻击成功率97.38\%，LFW中FID=18.27
\end{itemize}
\end{block}
...
\textcolor{hitblue}{修正章节安排描述，移除不同识别器架构，确保与实际实验内容一致。}
...
\textcolor{hitblue}{结论部分新增防御措施讨论以及未来研究方向展望}
