...
\textbf{问题1：方法有效性验证。}第三章与第四章分别提出了基于明晰扩散模型的TIA方法和基于换脸先验的MIA方法。本章需要通过定量与定性实验，验证这两种方法在实际攻击场景中的有效性，包括识别一致性（生成图像能否通过身份验证）、视觉质量（生成图像的真实感与自然性）以及身份保持度（生成图像与目标身份的相似程度）。

\textbf{问题2：方法优势分析。}相较于现有的模板逆向重建与模型反演方法，本文方法在生成质量、攻击成功率、计算效率等方面是否具有显著优势？哪些设计选择（如损失函数设计、微调策略、条件引导机制）对性能提升贡献最大？

\textbf{问题3：泛化与鲁棒性评估。}本文方法在不同数据集、不同识别器架构、不同威胁模型（白盒/灰盒/黑盒）下的性能表现如何？方法对遮挡、姿态变化、光照变化等实际场景中的扰动是否具有鲁棒性？

\textbf{问题4：关键因素分析。}通过消融实验，系统性地分解方法中各个模块与超参数的作用，量化每个设计选择对最终性能的影响，为方法的进一步优化与改进提供指导。

\subsection{本章贡献与组织结构}

针对上述研究问题，本章的主要贡献包括：

\textbf{（1）建立统一的评估协议。}设计了涵盖识别一致性、视觉质量、身份一致性、多样性与计算效率的多维度评估指标体系，为TIA与MIA方法的综合评估提供客观、可靠的标准。

\textbf{（2）系统性的实验验证。}在多个标准数据集（CelebA-HQ、LFW、MegaFace等）上进行了大规模实验，从多个维度验证了本文方法的有效性与优越性，并与现有代表性方法进行了详细对比。

\textbf{（3）深入的消融分析。}通过系统化的消融实验，量化了损失函数各项、网络架构选择、超参数配置等关键因素对性能的影响，揭示了方法成功的内在机制。

\textbf{（4）鲁棒性与泛化能力评估。}评估了方法在不同威胁模型、跨数据集、跨模型、以及面对防御机制时的性能表现，为实际部署中的安全风险评估提供了全面的分析。

\textbf{（5）失败案例分析与改进方向。}通过对典型失败案例的深入分析，识别了方法的局限性与潜在改进方向，为后续研究提供了有价值的启示。

本章的组织结构如下：第~\ref{sec:results_setup}节详细描述实验配置与评估指标的设计原理；第~\ref{sec:tia_results}节呈现TIA方法的实验结果与分析；第~\ref{sec:mia_results}节呈现MIA方法的实验结果与分析；第~\ref{sec:ablation}节进行消融研究；第~\ref{sec:comparison}节与现有方法进行详细对比并评估鲁棒性；第~\ref{sec:failure_analysis}节分析失败案例；第~\ref{sec:ethics_security}节讨论伦理与安全问题；最后，第~\ref{sec:results_summary}节对全章进行总结。

\subsection{可复现性保证}

为确保研究的可复现性，本章所有实验遵循以下原则：
\begin{itemize}
  \item \textbf{代码与脚本开源}：所有实验代码、训练脚本、评估脚本均已整理并随附于项目仓库的 \texttt{experiments/} 目录；
  \item \textbf{超参数完整记录}：每个实验的超参数配置（学习率、批大小、迭代次数、随机种子等）均以YAML或JSON格式记录在配置文件中；
  \item \textbf{随机种子固定}：所有涉及随机性的操作（数据采样、权重初始化、噪声注入）均设置固定随机种子（默认为42）；
  \item \textbf{环境依赖明确}：实验环境的Python版本、PyTorch版本、CUDA版本以及所有依赖库的版本均记录在 \texttt{requirements.txt} 与 \texttt{environment.yml} 中；
  \item \textbf{数据预处理标准化}：数据集的下载、预处理、分割方式均提供详细文档与自动化脚本；
  \item \textbf{统计显著性检验}：所有对比实验均进行至少5次重复，报告均值、标准差以及95\%置信区间，并进行配对t检验以评估差异的统计显著性。
\end{itemize}

通过严格的可复现性保证，本章实验结果不仅为本文方法的有效性提供了可靠证据，也为相关领域的后续研究提供了可参考的基准与标准。
...
\textbf{步骤1：人脸检测与对齐。}使用MTCNN~\cite{zhang2016joint}或dlib~\cite{king2009dlib}进行人脸检测，提取5个关键点（双眼中心、鼻尖、嘴角）。基于关键点计算相似变换矩阵，将人脸对齐至标准姿态（双眼水平线，鼻尖位于图像中心）。

\textbf{步骤2：裁剪与缩放。}根据关键点位置进行中心裁剪，保留完整的面部区域（包括额头、下巴、部分头发与背景）。将裁剪后的图像缩放至统一分辨率：扩散模型使用$256\times256$，人脸识别器使用$112\times112$（与ArcFace预训练分辨率一致）。

\textbf{步骤3：归一化与增强。}像素值归一化至$[0,1]$或$[-1,1]$（取决于模型输入要求）。训练阶段采用数据增强策略，包括随机水平翻转（概率0.5）、随机亮度与对比度调整（$\pm10\%$）、随机高斯模糊（核大小3，概率0.2），以提升模型的鲁棒性。测试阶段不使用数据增强。

\textbf{步骤4：模板提取。}对于TIA实验，使用预训练的ArcFace模型提取512维归一化嵌入向量作为模板。对于每个测试身份，从其所有图像中随机选择一张提取模板，剩余图像用于评估生成质量与身份一致性。

\subsection{实验环境与硬件配置}

\subsubsection{硬件环境}

所有实验在以下硬件配置的服务器上进行：
\begin{itemize}
  \item \textbf{GPU}：NVIDIA A100（40GB显存）或NVIDIA V100（32GB显存），根据实验批次与显存需求选择；
  \item \textbf{CPU}：Intel Xeon Gold 6248R（48核，3.0GHz基频）；
  \item \textbf{内存}：512GB DDR4 ECC；
  \item \textbf{存储}：2TB NVMe SSD（用于数据缓存与模型checkpoints）+ 10TB HDD（用于数据集长期存储）。
\end{itemize}

每次实验运行记录GPU型号、CUDA版本、显存峰值占用、训练/推理时间，以便后续的成本估算与效率分析。
...
\begin{itemize}
  ...
  \item \textbf{Python版本}：3.9.16；
  \item \textbf{深度学习框架}：PyTorch 2.0.1 + CUDA 11.8；
  \item \textbf{关键依赖库}：torchvision 0.15.2, numpy 1.24.3, pillow 9.5.0, opencv-python 4.7.0, scikit-learn 1.2.2, matplotlib 3.7.1, tensorboard 2.13.0；
  \item \textbf{人脸相关库}：insightface 0.7.3（用于ArcFace特征提取）, face-alignment 1.3.5（用于关键点检测）, lpips 0.1.4（用于感知质量评估）。
\end{itemize}

完整的依赖列表与版本信息记录在\texttt{experiments/requirements.txt}与\texttt{environment.yml}中，可通过\texttt{pip install -r requirements.txt}或\texttt{conda env create -f environment.yml}快速复现实验环境。

\subsubsection{训练与推理配置}

为提升训练效率并降低显存占用，实验采用以下技术：
\begin{itemize}
  \item \textbf{混合精度训练（AMP）}：使用PyTorch的自动混合精度（torch.cuda.amp），在保持数值稳定性的前提下将显存占用降低约40\%；
  \item \textbf{梯度累积}：当显存有限时，采用梯度累积策略（accumulation steps=2或4）模拟更大的有效批大小；
  \item \textbf{梯度检查点（Gradient Checkpointing）}：对扩散模型的UNet架构应用梯度检查点，以时间换空间，进一步降低显存需求；
  \item \textbf{分布式训练}：对于大规模实验，使用PyTorch的DistributedDataParallel（DDP）在多GPU上进行数据并行训练。
\end{itemize}

\textbf{TIA训练配置。}对于模板逆向攻击，扩散模型的微调采用以下超参数设置：
\begin{itemize}
  \item \textbf{批大小}：16（单卡）或64（4卡DDP），有效批大小通过梯度累积达到64；
  ...
  \item \textbf{LoRA配置}：秩$r=8$，缩放因子$\alpha=16$，应用于UNet的所有交叉注意力层与自注意力层；
  \item \textbf{损失函数权重}：$\lambda_{\text{denoise}}=1.0$，$\lambda_{\text{id}}=0.5$，$\lambda_{\text{reg}}=0.01$（详见第三章）；
  \item \textbf{采样步数}：训练时使用DDPM 1000步，推理时使用DDIM 50步加速采样；
  \item \textbf{引导强度}：Classifier-free guidance scale $w=7.5$。
\end{itemize}

\textbf{MIA训练配置。}对于模型反演攻击，换脸模型与LoRA微调采用以下设置：
\begin{itemize}
  \item \textbf{批大小}：8（单卡），有效批大小通过梯度累积达到32；
  \item \textbf{学习率}：$5\times10^{-6}$（LoRA参数），$1\times10^{-4}$（鉴别器），采用线性预热+余弦退火；
  \item \textbf{优化器}：AdamW，$\beta_1=0.5$，$\beta_2=0.999$，权重衰减$1\times10^{-4}$；
  \item \textbf{训练轮数}：10 epochs，每个epoch约30,000步（基于VGGFace2训练集）；
  \item \textbf{LoRA配置}：秩$r=16$，缩放因子$\alpha=32$，应用于换脸模型的编码器与解码器；
  \item \textbf{损失函数权重}：$\lambda_{\text{recon}}=1.0$，$\lambda_{\text{id}}=1.5$，$\lambda_{\text{adv}}=0.1$，$\lambda_{\text{perceptual}}=0.5$（详见第四章）；
  \item \textbf{嵌入策略}：使用textual inversion学习512维身份嵌入，学习率$1\times10^{-3}$；
  \item \textbf{预训练模型}：使用SimSwap作为基础换脸模型，预训练于FF++与CelebA-HQ。
\end{itemize}

\textbf{推理配置与加速策略。}推理阶段采用以下优化：
\begin{itemize}
  \item \textbf{批量推理}：每次推理生成batch\_size=16张图像，充分利用GPU并行计算能力；
  \item \textbf{模型量化}：对于资源受限场景，使用PyTorch的动态量化（torch.quantization）将模型精度从FP32降至INT8，推理速度提升1.8倍；
  \item \textbf{TensorRT优化}：对关键模块（UNet、换脸编码器）使用NVIDIA TensorRT进行图优化与内核融合，推理速度提升2.3倍；
  \item \textbf{Flash Attention}：对注意力模块应用Flash Attention~\cite{dao2022flashattention}，显存占用降低30\%，计算速度提升1.5倍；
  \item \textbf{模型剪枝}：对LoRA权重进行结构化剪枝，去除贡献度低于阈值（0.01）的通道，在保持性能的前提下参数量减少15\%。
\end{itemize}

\subsubsection{实验流程与数据管理}

\textbf{实验管理系统。}为确保实验的可追溯性与可复现性，本研究使用以下工具进行实验管理：
\begin{itemize}
  \item \textbf{配置管理}：使用Hydra框架管理超参数配置，每个实验自动生成独立的配置文件副本；
  \item \textbf{日志记录}：使用TensorBoard记录训练曲线（损失、指标、学习率），使用WandB进行云端同步与可视化；
  \item \textbf{版本控制}：使用Git管理代码版本，使用DVC（Data Version Control）管理大型模型checkpoints与数据集；
  \item \textbf{结果存储}：每个实验的输出（生成图像、评估指标、模型权重）存储在独立目录，命名格式为\texttt{exp\_\{date\}\_\{method\}\_\{config\_hash\}}。
\end{itemize}

\textbf{数据缓存与加载优化。}针对大规模数据集的I/O瓶颈，采用以下优化策略：
\begin{itemize}
  \item \textbf{预处理缓存}：首次运行时将预处理后的图像（对齐、裁剪、缩放）缓存至SSD，后续训练直接加载缓存；
  \item \textbf{内存映射}：使用numpy的memmap或HDF5格式存储大型特征矩阵（如预提取的ArcFace嵌入），实现零拷贝加载；
  \item \textbf{多进程数据加载}：使用PyTorch的DataLoader with num\_workers=8，预取缓冲区大小为64；
  \item \textbf{在线数据增强}：在GPU上进行数据增强操作（使用Kornia库），避免CPU-GPU数据传输开销。
\end{itemize}
...
\textbf{（1）训练时间。}记录模型微调所需的总训练时间（单位：GPU小时），以及每个epoch的平均时间。

\textbf{（2）推理时间。}记录生成单张图像的平均推理时间（单位：毫秒或秒），包括前处理、前向传播与后处理的总时间。

\textbf{（3）显存占用。}记录训练与推理阶段的峰值显存占用（单位：GB）。

\textbf{（4）可训练参数量。}对于使用LoRA微调的方法，统计额外引入的可训练参数量，并计算相对于全参数微调的参数效率（参数量比例）。

\textbf{（5）查询次数（灰盒/黑盒场景）。}记录每个目标身份平均需要查询目标分类器的次数，以评估攻击的查询效率。

\subsubsection{评估协议与实验设置}

\textbf{评估模式分类。}根据攻击者对目标系统的访问权限，本研究在三种威胁模型下进行评估：

\textbf{白盒模式}：攻击者完全了解目标识别器的架构、参数与训练数据，可以直接计算梯度进行优化。本模式下评估方法的理论上界性能。

\textbf{灰盒模式}：攻击者了解目标识别器的架构但不知道具体参数，可以访问模型的输出概率分布或嵌入向量。本模式下使用替代模型（surrogate model）进行攻击，评估方法的迁移能力。

\textbf{黑盒模式}：攻击者仅能查询目标系统获得最终预测结果（Top-k类别或相似度分数），无法访问中间层输出。本模式下使用基于查询的优化方法（如自然进化策略NES）。
...
为确保实验结果的可靠性与统计显著性，本研究采用以下统计分析方法：

\textbf{（1）重复实验与置信区间。}每个实验配置重复5次，使用不同的随机种子（42, 43, 44, 45, 46），报告均值$\mu$与标准差$\sigma$。对于关键指标，还报告95\%置信区间$[\mu - 1.96\frac{\sigma}{\sqrt{5}}, \mu + 1.96\frac{\sigma}{\sqrt{5}}]$。

\textbf{（2）配对t检验。}对于方法间的性能对比，使用配对t检验（paired t-test）评估差异的统计显著性。显著性水平设为$\alpha=0.05$，当$p<0.05$时认为差异显著（标记为*），当$p<0.01$时认为差异极显著（标记为**）。

\textbf{（3）效应量分析。}除p值外，还报告Cohen's d效应量，量化方法间差异的实际重要性。按照惯例，$|d|<0.2$为小效应，$0.2\leq|d|<0.8$为中等效应，$|d|\geq0.8$为大效应。

\textbf{（4）箱线图与分布可视化。}对于连续型指标（如余弦相似度、LPIPS），使用箱线图展示分布的中位数、四分位数、极值与离群点，以全面呈现数据的分布特征。

\textbf{多重比较校正。}当进行多组方法对比时（如消融实验中测试多个超参数配置），使用Bonferroni校正或Holm-Bonferroni校正控制族错误率（Family-Wise Error Rate, FWER），避免因多重检验导致的假阳性。调整后的显著性水平为$\alpha'=\alpha/m$，其中$m$为比较次数。

\textbf{非参数检验。}对于不满足正态分布假设的数据（如经Shapiro-Wilk检验后$p<0.05$），使用非参数统计方法：
\begin{itemize}
  \item \textbf{Wilcoxon符号秩检验}：替代配对t检验，用于成对样本的中位数差异检验；
  \item \textbf{Kruskal-Wallis H检验}：替代单因素方差分析（ANOVA），用于多组独立样本的分布差异检验；
  \item \textbf{Mann-Whitney U检验}：替代独立样本t检验，用于两组独立样本的分布差异检验。
\end{itemize}

\textbf{效应量的标准化报告。}除了显著性检验，本研究还系统报告效应量，以量化实际意义：
\begin{itemize}
  \item \textbf{Cohen's d}：用于t检验，$d=\frac{\mu_1-\mu_2}{\sigma_{\text{pooled}}}$，其中$\sigma_{\text{pooled}}=\sqrt{\frac{(n_1-1)\sigma_1^2+(n_2-1)\sigma_2^2}{n_1+n_2-2}}$；
  \item \textbf{Cliff's Delta}：用于非参数检验，$\delta=\frac{\#(x_i>y_j)-\#(x_i<y_j)}{n_1\times n_2}$，范围$[-1,1]$，$|\delta|<0.147$为可忽略效应，$0.147\leq|\delta|<0.33$为小效应，$0.33\leq|\delta|<0.474$为中等效应，$|\delta|\geq0.474$为大效应；
  \item \textbf{$\eta^2$（Eta squared）}：用于方差分析，$\eta^2=\frac{SS_{\text{between}}}{SS_{\text{total}}}$，表示组间差异占总方差的比例。
\end{itemize}

\textbf{可视化规范。}所有统计图表遵循以下规范：
\begin{itemize}
  \item 误差棒表示95\%置信区间（而非标准差或标准误），置信区间计算基于bootstrap方法（重采样1000次）；
  \item 箱线图的箱体表示四分位数（Q1, Q2, Q3），须须延伸至1.5倍IQR（四分位距），离群点单独标记；
  \item 显著性标记：$*$表示$p<0.05$，$**$表示$p<0.01$，$***$表示$p<0.001$，n.s.表示不显著；
  \item 所有图表提供色盲友好的配色方案（使用ColorBrewer或Seaborn的默认调色板）。
\end{itemize}

\textbf{数据透明度与开放获取。}本研究承诺：
\begin{itemize}
  \item 所有实验的原始数据（包括每次运行的详细指标）以CSV或JSON格式存储在项目仓库的\texttt{results/raw\_data/}目录；
  \item 统计分析脚本（使用Python的scipy、statsmodels库）开源在\texttt{scripts/statistical\_analysis.py}；
  \item 生成的所有图表提供高分辨率矢量格式（PDF或SVG）与源代码（Matplotlib或Seaborn脚本）。
\end{itemize}

通过上述严格的统计分析方法与透明度保障，本章的实验结论具有高度的可信度与可复现性，为领域内的后续研究提供了坚实的基准。
...
\begin{table}[htbp]
  \centering
  \caption{TIA方法的定量性能对比（LFW测试集，500个身份，每个身份生成10张图像）}
  \label{tab:tia_quantitative}
  \begin{tabular}{lcccccc}
    \hline
    \textbf{方法} & \textbf{TAR@FAR(1e-3)} & \textbf{TAR@FAR(1e-4)} & \textbf{AvgCosSim}$\uparrow$ & \textbf{FID}$\downarrow$ & \textbf{LPIPS}$\downarrow$ & \textbf{IS}$\uparrow$ \\
    \hline
    直接采样（无条件） & 0.12 ± 0.02 & 0.05 ± 0.01 & 0.31 ± 0.03 & 67.4 ± 2.1 & 0.512 ± 0.021 & 2.84 ± 0.12 \\
    文本引导采样 & 0.28 ± 0.03 & 0.14 ± 0.02 & 0.48 ± 0.02 & 52.3 ± 1.8 & 0.467 ± 0.018 & 3.21 ± 0.15 \\
    GAN Inversion~\cite{xia2022gan} & 0.54 ± 0.04 & 0.32 ± 0.03 & 0.63 ± 0.03 & 41.2 ± 2.3 & 0.398 ± 0.022 & 3.67 ± 0.18 \\
    DeepInversion~\cite{yin2020dreaming} & 0.61 ± 0.03 & 0.41 ± 0.04 & 0.68 ± 0.02 & 38.9 ± 1.9 & 0.376 ± 0.019 & 3.82 ± 0.14 \\
    NBNet~\cite{mai2021neural} & 0.73 ± 0.02 & 0.56 ± 0.03 & 0.74 ± 0.02 & 32.1 ± 1.5 & 0.342 ± 0.016 & 4.15 ± 0.13 \\
    \hline
    \textbf{TIA（无微调）} & 0.43 ± 0.02 & 0.21 ± 0.03 & 0.62 ± 0.01 & 45.1 ± 1.7 & 0.401 ± 0.017 & 3.48 ± 0.11 \\
    \textbf{TIA（潜在优化NES）} & 0.68 ± 0.03 & 0.45 ± 0.04 & 0.71 ± 0.02 & 38.7 ± 1.6 & 0.362 ± 0.015 & 3.89 ± 0.12 \\
    \textbf{TIA（LoRA, r=4）} & 0.84 ± 0.02 & 0.68 ± 0.03 & 0.79 ± 0.01 & 27.5 ± 1.3 & 0.328 ± 0.014 & 4.32 ± 0.10 \\
    \textbf{TIA（LoRA, r=8）} & \textbf{0.92 ± 0.01**} & \textbf{0.83 ± 0.01**} & \textbf{0.83 ± 0.01**} & \textbf{22.3 ± 1.1**} & \textbf{0.313 ± 0.012**} & \textbf{4.58 ± 0.09**} \\
    \textbf{TIA（LoRA, r=16）} & 0.93 ± 0.01** & 0.86 ± 0.01** & 0.84 ± 0.01** & 21.7 ± 1.0** & 0.305 ± 0.011** & 4.63 ± 0.08** \\
    \textbf{TIA（混合:NES+LoRA）} & 0.94 ± 0.01** & 0.88 ± 0.01** & 0.85 ± 0.01** & 21.1 ± 0.9** & 0.298 ± 0.010** & 4.71 ± 0.07** \\
    \hline
  \end{tabular}
  \begin{tablenotes}
    \item[1] 所有结果基于5次重复实验，报告均值±标准差。**表示与最佳基准方法（NBNet）相比差异极显著（$p<0.01$，配对t检验）。
    \item[2] TAR@FAR在MegaFace-subset (100K distractors)上计算。$\uparrow$/$\downarrow$分别表示越高/越低越好。
  \end{tablenotes}
\end{table}
...
\textbf{（4）秩的选择影响性能。}对比不同LoRA秩的配置，可以发现r=8已经接近性能饱和点。从r=4到r=8，TAR@FAR(1e-3)提升9.5\%（0.84→0.92），而从r=8到r=16，仅提升1.1\%（0.92→0.93）。考虑到更大的秩会增加训练与推理成本（参数量与时间），r=8是一个较优的折中选择。

\textbf{（5）混合策略带来边际收益。}结合NES潜在优化与LoRA微调的混合策略在所有指标上均取得了最佳性能，但相比单独使用LoRA（r=8），提升幅度有限（TAR@FAR(1e-3): 0.92→0.94，提升2.2\%）。考虑到混合策略显著增加了推理时间（180ms→320ms，见表~\ref{tab:tia_efficiency}），其实用性取决于具体的应用场景对性能与效率的权衡。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccc}
    ...
    \hline
    ArcFace / ArcFace（白盒） & 0.92 ± 0.01 & 0.83 ± 0.01 & 0.89 ± 0.02 \\
  ...
  \end{tabular}
  \begin{tablenotes}
    \item[1] Ensemble识别器：综合ArcFace、CosFace、AdaFace的平均预测，模拟多识别器防御场景。
...
\textbf{（2）Ensemble识别器提供更强防御。}当使用三个识别器的集成预测进行验证时，TAR@FAR(1e-3)进一步下降至0.75，表明多识别器集成是一种有效的防御策略。
...
\begin{figure}[!htbp]
  ...
  \caption{TIA方法的定性结果对比。每行展示同一目标身份的：(a)真实图像（参考），(b)GAN Inversion，(c)NBNet，(d)TIA无微调，(e)TIA LoRA(r=8)，(f)TIA混合策略。红框标注成功案例，蓝框标注失败案例。}
...
\textbf{（3）混合策略在极端案例下表现更佳。}对于某些困难身份（如第4行的侧脸、第6行的遮挡），混合策略（图f）相比单独LoRA（图e）能够生成更准确的面部结构与身份特征，尽管在简单案例下两者差异不明显。
...
\subsubsection{不同威胁模型下的性能}

MIA的实际威胁取决于攻击者对目标分类器的访问权限。本研究在三种威胁模型（白盒、灰盒、黑盒）下评估了MIA方法的性能，结果见表~\ref{tab:mia_threat_models}。

\begin{table}[htbp]
  \centering
  \caption{MIA方法在不同威胁模型下的性能（VGGFace2测试集）}
  \label{tab:mia_threat_models}
  \begin{tabular}{lcccc}
    \hline
    \textbf{威胁模型} & \textbf{访问权限} & \textbf{Top-1 Acc} & \textbf{ID-Pres} & \textbf{查询次数} \\
    \hline
    白盒 & 梯度+logits+嵌入 & 0.82 ± 0.01 & 0.83 ± 0.01 & 0 \\
    灰盒（Logits） & Logits（概率分布） & 0.76 ± 0.02 & 0.78 ± 0.02 & 0 \\
    灰盒（Top-k） & Top-k类别+置信度 & 0.71 ± 0.02 & 0.74 ± 0.02 & 0 \\
    黑盒（Score-based） & 相似度分数 & 0.64 ± 0.03 & 0.69 ± 0.03 & 5,230 ± 420 \\
    黑盒（Decision-based） & 是/否决策 & 0.53 ± 0.03 & 0.61 ± 0.03 & 12,840 ± 680 \\
    \hline
  \end{tabular}
  \begin{tablenotes}
    \item[1] 查询次数：平均每个目标身份需要查询目标分类器的次数（白盒与灰盒无需查询）。
    \item[2] 黑盒场景使用自然进化策略（NES）进行查询优化，查询预算设为20,000次。
  \end{tablenotes}
\end{table}

从表~\ref{tab:mia_threat_models}可以看出：

\textbf{（1）白盒场景下MIA达到最高成功率。}当攻击者能够访问目标分类器的梯度、logits与嵌入向量时，MIA的Top-1 Acc达到0.82，ID-Pres达到0.83。这是因为梯度信息能够直接指导生成过程，使生成图像在特征空间中快速逼近目标类别的中心。

\textbf{（2）灰盒场景性能适度下降。}当仅能访问logits（概率分布）时，Top-1 Acc下降至0.76（相对下降7.3\%）；当仅能访问Top-k类别与置信度时，进一步下降至0.71（相对下降13.4\%）。尽管如此，灰盒MIA仍显著优于现有基准方法在白盒场景下的性能（0.71 vs 0.68），证明了MIA方法的鲁棒性。

\textbf{（3）黑盒场景面临显著挑战。}在仅能通过查询获得相似度分数或是/否决策的黑盒场景下，MIA的性能显著下降（Top-1 Acc分别为0.64和0.53）。这是因为黑盒优化需要大量查询（平均5,000-13,000次）来估计梯度方向，且容易陷入局部最优。然而，即使在最严格的decision-based黑盒场景下，MIA的Top-1 Acc（0.53）仍显著高于现有黑盒方法（如BREP-MI的0.41）。

\textbf{（4）查询效率存在提升空间。}黑盒MIA的查询次数较高，限制了其在查询受限场景下的应用。未来可以通过改进优化算法（如使用更高效的Zeroth-Order优化方法、引入查询缓存机制）来降低查询成本。
