...
\subsection{定性结果与示例目录}
定性结果分为：成功样本、部分成功样本与失败样本（示例见 `results/qual/`）：
\begin{itemize}
  \item `results/qual/success/`：生成样本与目标模板高度匹配的示例；\
  \item `results/qual/partial/`：样貌近似但细节（发型、光照）不一致；\
  \item `results/qual/failure/`：在强遮挡或极端角度下失败的示例。 
\end{itemize}

图~\ref{fig:qual_examples} 展示若干典型案例（目标模板 | 生成样本 | 真实样本 | 识别得分）：
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{images/qual_grid.png}
...
\section[可复现性与提供的产物]{可复现性与提供的产物}
为便于复现，我们在仓库中提供：
\begin{itemize}
  \item 完整训练/微调脚本：`experiments/train_lora.sh`、`scripts/finetune_lora.py`；\
  \item 批量推理与评估脚本：`experiments/run_infer.sh`、`experiments/eval_metrics.py`；\
  \item 结果目录示例：`results/main/`（CSV/PNG）、`results/qual/`（示例图）；\
  \item 关键超参数配置样例：`experiments/configs/main.yaml`；\
  \item 随机种子与依赖：`experiments/seed_info.txt`、`experiments/requirements.txt`。 
\end{itemize}

示例运行命令：
\begin{verbatim}
# LoRA 微调
bash experiments/train_lora.sh --config experiments/configs/main.yaml --out experiments/run001

# 批量推理并评估
bash experiments/run_infer.sh --ckpt experiments/run001/merged.ckpt --templates data/templates/test_list.txt --out results/main/run001
python experiments/eval_metrics.py --gen results/main/run001 --gallery data/gallery/ --recognizer models/arcface.ckpt --out results/main/run001/metrics.json

# 生成展示图
python experiments/plot_examples.py --result_dir results/qual/ --out figures/qual_examples.png
\end{verbatim}
