...
生成器：基于隐空间扩散（LDM/EDM）与换脸骨干的预训练模型，代码实现基于本工作仓库中的模块。\
...
统计报告：对每个实验配置报告均值 ± 标准差；若样本数较小，同时报告 95\% 置信区间。
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{images/SIFT.png}
...
\item 提供绘图脚本，直接从 CSV/JSON 读取并生成高分辨率图像用于论文排版。
\end{itemize}
