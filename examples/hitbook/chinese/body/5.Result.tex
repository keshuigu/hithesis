...
\begin{itemize}
  ...
  \item \textbf{预训练模型}：使用REFace作为基础换脸模型，预训练于FF++与CelebA-HQ。
\end{itemize}
...
\subsubsection{动态权衡参数策略}

训练过程中采用动态线性权重$\lambda$来平衡生成图像与特征模板之间的相似度和生成图像与原始输入图像之间的相似度。如第三章所述，TIA方法的损失函数包含两个核心部分：像素空间的均方误差损失和特征空间的嵌入一致性损失。动态权衡策略的核心思想是在训练初期优先保证图像的基础质量和结构，随着训练深入逐渐增强身份特征的约束。

具体实现上，$\lambda$的初始值设置为0，在训练过程中逐渐增加到100。调整策略为：模型每学习1000张图像将$\lambda$增加0.01，在10000次迭代后达到最大值100并保持不变。这种渐进式的权重调整策略使模型能够首先建立稳定的图像生成能力，然后在此基础上逐步强化身份一致性约束，避免了过早引入强身份约束导致的训练不稳定和图像质量下降问题。
...
本研究引入基于换脸技术的先验模型用于MIA攻击，相比使用通用扩散模型具有显著优势。换脸模型通过在大规模真实人脸数据上的训练，学习了人脸在不同身份、姿态和光照条件下的结构、纹理和表情变化规律。这种强大的人脸建模能力为后续的身份迁移和图像生成提供了丰富的先验知识。
