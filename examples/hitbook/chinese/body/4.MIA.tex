...
\textbf{分阶段训练策略：}标签条件嵌入层替换原始身份编码器后，模型初期生成的图像呈现随机噪声状态，缺乏基本的人脸结构。为实现稳定优化，本文采用两阶段训练策略，遵循"先恢复生成能力，再适配分类器"的原则。

生成能力恢复阶段:冻结换脸模型主干与扩散U-Net,仅训练标签条件嵌入层$\mathcal{E}_\psi$与少量LoRA参数。该阶段的核心目标是使模型恢复生成自然人脸的能力,优化目标为:
\begin{equation}
    \mathcal{L}_{\text{stage1}} = \mathcal{L}_{\text{prior}} + \mathcal{L}_{\text{perc}} + \mathcal{L}_{\text{emb-norm}},
\end{equation}
即强扩散先验约束与感知质量约束,确保生成图像重回自然人脸流形。此阶段不引入分类器损失与身份对比损失,避免在生成能力未恢复时施加矛盾的优化信号。嵌入层学习将类别标签映射为能够触发有效生成的嵌入向量,LoRA参数适配新的嵌入分布。

该阶段的切换时机通过以下三个判据联合确定:(1)训练步数达到预设阈值(通常500-1000步);(2)生成图像的Fréchet Inception Distance(FID)降至合理范围(如FID<100,表明生成图像具备基本的人脸特征);(3)人工抽查生成样本,确认已呈现清晰的五官结构与自然肤色。实践中,步数判据为主要依据,FID判据为辅助验证,人工检查用于最终确认。若FID持续高于阈值,可延长该阶段训练至1500-2000步。

分类器适配与身份控制阶段：在生成能力恢复后，解冻U-Net注意力层与残差块卷积，引入完整损失函数$\mathcal{L}_{\text{total}}$（式~\ref{eq:mia_total_loss}）。该阶段同时优化攻击有效性与生成质量：分类器损失$\mathcal{L}_{\text{cls}}$驱动生成图像被目标分类器识别，身份对比损失$\mathcal{L}_{\text{id}}$建立类别标签与视觉身份的映射，先验损失$\mathcal{L}_{\text{prior}}$与感知损失$\mathcal{L}_{\text{perc}}$保持生成质量。各损失权重通过任务不确定性框架自动学习，LoRA参数采用较小学习率以避免遗忘预训练知识。

该分阶段策略确保模型先建立稳定的生成基础，再逐步适配目标分类器，避免优化初期的梯度冲突导致的训练崩溃。
...
算法~\ref{alg:mia_stage1}和算法~\ref{alg:mia_stage2}给出了两阶段训练的详细流程。

\begin{algorithm}[htbp]
  \caption{MIA阶段1：生成能力恢复}
  \label{alg:mia_stage1}
  \begin{algorithmic}[1]
    \REQUIRE 训练集$\mathcal{D}=\{(x_i, y_i)\}$，换脸模型$G_\phi$（主干冻结），嵌入层$\mathcal{E}_\psi$，学习率$\eta_{\text{emb}}, \eta_{\text{lora}}$
    \ENSURE 训练后的嵌入层参数$\psi$与初步的LoRA参数
    \STATE 初始化嵌入层参数$\psi$，初始化少量LoRA参数$\{A_\ell, B_\ell\}$（仅参考注意力层）
    ...
    \STATE 通过去噪网络预测噪声：$\hat{\epsilon}^{(i)} = \epsilon_{\theta}(z_t^{(i)}, t^{(i)}, e_{\text{id}}^{(i)})$
    \STATE 计算扩散先验损失：$\mathcal{L}_{\text{prior}}^{(i)} = w(t^{(i)}) \cdot (1 - \cos(\epsilon^{(i)}, \hat{\epsilon}^{(i)}))$
    \STATE 生成完整图像：$\hat{x}^{(i)} = G_{\phi}(e_{\text{id}}^{(i)}, x_{\text{src}}^{(i)})$（完整DDIM采样）
    \STATE 计算感知损失：$\mathcal{L}_{\text{perc}}^{(i)}$（式~\ref{eq:mia_perc_hierarchical}）
    \STATE 计算嵌入归一化损失：$\mathcal{L}_{\text{emb-norm}}^{(i)} = (\|e_{\text{id}}^{(i)}\|_2 - 1)^2$
    \ENDFOR
    \STATE 计算批平均损失：
    \STATE \quad $\mathcal{L}_{\text{stage1}} = \frac{1}{B}\sum_{i=1}^B [\mathcal{L}_{\text{prior}}^{(i)} + \mathcal{L}_{\text{perc}}^{(i)} + \mathcal{L}_{\text{emb-norm}}^{(i)}]$
...
\begin{algorithm}[htbp]
  ...
  \begin{algorithmic}[1]
    ...
    \STATE 其中$\bar{\mathcal{L}}_i = \frac{1}{B}\sum_{j=1}^B \mathcal{L}_i^{(j)}$
    \STATE 更新参数：
