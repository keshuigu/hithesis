...
\chapter{基于人脸替换的标签重建人脸方法}[Face Image Inverse Reconstruction from Class Labels via Face Replacement Model Prior Migration]\label{chap:MIA}

\section{引言}[Introduction]

标签重建人脸攻击旨在从训练完成的分类模型中重建其训练数据中目标身份的人脸图像。传统基于梯度优化的方法直接在像素空间进行优化，面临生成质量低下、优化效率不足等问题。近年来引入生成式先验可提升攻击效果，但现有工作在身份控制精度与生成保真度的平衡上仍存在不足。

本章提出基于人脸替换的标签重建人脸方法（SwapMIA），将扩散人脸替换模型应用于从类别标签重建人脸图像的任务。该模型具有显式的身份属性解耦机制，但其标准推理流程依赖真实目标图像，而任务场景中仅拥有类别标签。SwapMIA设计标签条件嵌入层、低秩适配微调、渐进式训练策略、多目标优化框架与分类器引导推理机制，将离散标签映射为连续身份嵌入，实现从类别标签到高保真攻击样本的生成。

本章首先形式化定义问题与威胁模型，随后阐述人脸替换先验模型的选择与适配改造、标签条件嵌入层设计、低秩适配微调策略、多目标优化损失函数框架与渐进式三阶段训练策略，最后给出完整的训练算法与分类器引导推理流程。基于上述方法，本章在VGGFace2、CelebA等标准数据集上进行系统实验，针对ArcFace、IR152与Face.evoLVe等不同架构的目标分类器评估攻击性能，通过与现有代表性方法的对比实验验证SwapMIA在攻击成功率与生成保真度上的优势，并通过消融实验量化各关键设计模块的性能贡献。
...
标签重建人脸方法针对基于分类的端到端人脸识别模型。这类系统将人脸识别建模为多分类问题，设分类模型为 $F_\theta: \mathbb{R}^{H \times W \times C} \to \mathbb{R}^N$，其中 $N$ 为训练类别数，$\theta$ 为模型参数。模型通过softmax层输出各类别的概率分布，在训练过程中学习将输入图像映射到对应身份类别。给定目标类别 $y \in \{1, 2, \ldots, N\}$，攻击者的目标是生成图像 $\hat{x}$，使其同时满足两个核心特性。

首先是分类器认同，即生成图像应使分类器在目标类别上产生高置信度输出。形式化地，要求 $F_\theta(\hat{x})$ 在目标类别 $y$ 上的输出值显著高于其他类别，即 $F_\theta(\hat{x})_y > F_\theta(\hat{x})_j, \forall j \neq y$。在实际攻击场景中，通常要求满足 $F_\theta(\hat{x})_y \geq \tau$，其中 $\tau$ 为置信度阈值，在人脸识别模型中常设定为0.5到0.9之间。高置信度输出表明生成图像被分类器强烈认定为目标身份，意味着攻击成功恢复了训练数据中该类别的关键特征。其次是视觉真实性，即生成图像 $\hat{x}$ 应在感知质量上与真实人脸图像无明显差异，具有自然的面部特征和纹理细节，可形式化为要求 $\hat{x}$ 属于自然人脸图像流形 $\mathcal{M}_{\text{natural}}$。

\subsubsection{攻击者知识假设}

本章在白盒威胁模型下开展研究，攻击者拥有如下知识：

（1）目标类别标签。攻击者仅拥有目标身份在分类型人脸识别模型中对应的离散类别索引 $y \in \{1,2,\ldots,N\}$，不持有任何目标身份的真实人脸图像或特征嵌入向量，即攻击在仅凭标签的条件下进行。

（2）目标模型完全知识。攻击者掌握目标分类器 $F_\theta$ 的完整网络架构与训练参数，且可进行前向推理与反向梯度计算，即 $\nabla_x F_\theta(x)$ 可获取。该假设对应白盒攻击场景，代表最强威胁下的模型隐私泄露上界，对应模型开源、内部泄露或逆向工程成功的现实场景。

（3）公开人脸数据集。攻击者可使用与目标系统训练集无重叠的公开人脸数据集（如VGGFace2、CelebA）训练生成模型，但无法访问目标系统的原始训练样本或对应标注信息。

（4）预训练生成先验。攻击者可访问公开的预训练生成模型（如扩散式人脸替换骨干网络），以其作为生成先验进行迁移适配，但该先验并非针对目标模型定制训练。

\subsubsection{问题形式化}

形式化地，标签重建人脸问题可建模为如下优化问题：
\begin{equation}\label{eq:mia_optimization}
\hat{x} = \arg\max_x \; F_\theta(x)_y + \lambda_{perc}\mathcal{L}_{perc}(x) - \lambda_{reg}R(x)
\end{equation}

其中第一项为目标类别的分类器输出，通过最大化该项驱动生成图像被识别为目标身份；第二项 $\mathcal{L}_{perc}$ 为感知质量约束，常采用LPIPS\cite{zhang2018unreasonable}等基于深度特征的感知相似度度量，确保生成图像具有自然的视觉外观；第三项 $R(\cdot)$ 为正则化项，防止生成图像偏离自然图像流形；$\lambda_{perc}$ 和 $\lambda_{reg}$ 为权衡系数。与模板重建人脸不同，标签重建人脸方法的优化目标是最大化分类器输出而非最小化特征距离，这使得攻击过程需要直接利用分类器的梯度信息 $\nabla_x F_\theta(x)_y$ 引导生成过程。

攻击成功判据同样包括多个层次：强成功是指生成图像的分类置信度超过预设阈值 $F_\theta(\hat{x})_y \geq \tau$；弱成功是指目标类别的输出值虽未达到阈值但显著高于其他类别，表明攻击部分恢复了目标身份的判别特征；视觉质量成功是指生成图像具有真实的人脸外观，能够通过人类视觉检验。此外，在实际攻击场景中，还需要考虑生成样本的多样性，即对同一目标类别生成的多张图像应在姿态、表情、光照等属性维度上呈现合理的变化，而非完全一致的复制。

\section{基于人脸替换的标签重建人脸算法设计}[Face-Swapping-Prior-Based Inversion Architecture and Multi-Objective Loss]
\label{sec:mia_architecture}

SwapMIA围绕人脸替换模型的先验迁移展开设计，依次解决生成先验的选择与适配、标签条件的整体结构建模、多目标损失优化以及渐进式训练与推理四个核心问题。本节以此为主线，依次阐述各子模块的设计思路与实现细节。

\subsection{基于人脸替换模型的生成先验构建}

SwapMIA的生成模块需要同时具备三项能力：高质量的身份特征迁移、显式的身份-属性解耦、以及端到端的可微分训练支持。传统类条件生成方法将类别标签直接作为条件输入，身份与属性信息隐式耦合在同一条件空间，存在三方面固有局限：（1）身份与姿态、表情、光照等属性维度缺乏明确分离，难以在保证身份一致性的同时生成属性多样化的样本；（2）此类方法通常需在目标数据集上从零训练，缺乏大规模人脸先验，生成图像的面部结构合理性与细节保真度均受限；（3）身份表示与属性生成两个复杂任务须联合优化，训练难度高且易出现模式崩溃。针对上述问题，SwapMIA引入扩散模型的人脸替换方法REFace~\cite{baliah2025realistic}作为生成骨干，利用其参考注意力机制所实现的显式身份-属性解耦能力构建生成先验。REFace将身份信息单独编码为嵌入向量，并通过交叉注意力层注入生成过程，姿态、表情、光照等属性信息由源图像自然提供，两者在特征空间中相互独立，从而将身份控制从"联合生成身份与属性"简化为"仅控制身份迁移"，并借助大规模人脸预训练积累的生成先验，有效提升重建图像的保真度与身份一致性。

然而，REFace的标准流程面向图像到图像的人脸替换任务，依赖真实目标图像提供身份参考，而标签重建人脸场景中攻击者仅拥有离散类别标签，无法获取任何真实目标图像。这种输入模态的根本差异带来三方面技术障碍：（1）标准流程的身份嵌入来自图像编码器的实时提取，须从离散标签生成等价的连续嵌入替代；（2）标签映射网络的输出分布与ArcFace在真实人脸上学习的嵌入分布存在显著偏差，直接替换将导致生成模型失效；（3）标签重建任务须同时优化分类器置信度，而原始任务仅优化感知相似度与身份保持度，两类目标在生成空间中可能相互竞争。针对这三方面障碍，本章提出系统性的三层改造方案：

（1）标签条件嵌入层设计：采用多层感知机将类别标签映射为连续身份嵌入向量，替代原有的ArcFace图像编码器，建立从离散标签空间到连续身份嵌入的可学习桥梁；

（2）LoRA参数高效微调：针对标签嵌入输出分布与ArcFace真实嵌入分布的偏差，在冻结预训练主干的前提下，为关键层引入低秩可训练增量，以极小参数开销完成嵌入分布适配；

（3）渐进式训练与多目标优化：通过图像条件预热、混合条件过渡与纯标签条件适配三个阶段实现平滑的模态迁移，同时采用任务不确定性加权框架统一管理多类优化目标。

以下各小节依次详细阐述这三层改造模块对应的整体结构、损失函数设计与训练推理策略。
...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[scale=0.75, transform shape, node distance=1.3cm, >=stealth, thick,
    ...
    \path (current bounding box.north east) ++(1cm, 0);
    % 图例（右下角，上下摆放）
    \node[anchor=north east, draw=none, fill=white, inner sep=5pt, rounded corners=3pt, font=\footnotesize]
  ...
  \end{tikzpicture}
  \caption{标签重建人脸方法整体架构}
  \label{fig:mia_architecture}
\end{figure}

SwapMIA的整体架构如图~\ref{fig:mia_architecture}所示，由标签嵌入层$\mathcal{E}_\psi$、人脸替换生成模型（VAE编码器、U-Net+LoRA、VAE解码器）、下游评估模块（分类器$F_\theta$和ArcFace编码器$E_{\text{id}}$）以及六个损失函数组成。
...
直接对人脸替换模型进行全参数微调面临参数量巨大、过拟合风险高等挑战。低秩适配（Low-Rank Adaptation，LoRA）技术通过在冻结预训练权重的前提下引入低秩可训练增量，仅需训练原模型1\%-5\%的参数量，能够有效降低过拟合风险并保留预训练模型的生成先验知识。

根据人脸替换模型的架构特点与身份控制机制，本章将LoRA应用于以下三类关键模块。这些模块的选择基于对身份信息流动路径的系统分析：参考注意力层直接处理身份嵌入的编码，是身份信息注入的入口；U-Net注意力层负责特征的全局聚合与身份信息的空间传播，决定身份信息在生成过程中的传递效果；残差块卷积层调整通道间的特征映射，影响身份特征的具体表达。这些模块对身份控制最为敏感，微调它们可以最大化地适配新的身份嵌入分布，同时最小化对生成质量的负面影响。
...
本小节系统设计面向标签重建人脸的多目标优化损失函数框架。标签重建人脸需要同时优化多个相互关联但可能存在冲突的目标：保持预训练扩散模型的生成质量、提高对目标分类器的攻击有效性、确保生成图像的身份一致性、维持视觉真实性以及防止模型过拟合。本章通过精心设计六个损失组件实现上述目标的统一优化，并采用任务不确定性加权框架实现各损失项的动态平衡。以下分别阐述各损失组件的设计原理与技术细节，最后给出完整的多目标优化框架。

\subsubsection{扩散先验损失}

扩散先验损失确保LoRA微调不破坏预训练人脸替换模型的生成能力，保持生成图像的自然性与真实性。本章沿用扩散模型的标准去噪目标（详见第\ref{sec:diffusion_models}节），采用余弦相似度度量预测噪声与真实噪声的一致性：
...
\subsubsection{分类器引导损失与特征空间正则化}

分类器引导损失是标签重建人脸的核心驱动力，旨在使生成图像被目标分类器高置信度地识别为目标类别。传统交叉熵损失$-\log p_y$仅最大化目标类别概率，易导致生成图像在决策边界附近徘徊，难以实现鲁棒的攻击效果。本章采用Li等人\cite{li2024model}提出的top-k max-margin损失，该损失通过将传统max-margin损失\cite{carlini2017towards}中仅考虑单个最大非目标类别logit的"hard max"策略替换为考虑前$k$个最大非目标类别logit的"top-k平均"策略，更充分地利用分类器输出的软标签信息，增强生成图像与决策边界的判别性距离。
...
为进一步稳定优化过程，本文采用Nguyen等人\cite{nguyen2023re}提出并由Li等人\cite{li2024model}改进的p-reg损失，在特征空间对生成图像进行正则化约束。该损失通过最小化生成图像的特征表示与预计算的类别特征中心之间的距离，避免优化过程中的振荡。设$p_x = F_\theta^{\text{feat}}(\hat{x}) \in \mathbb{R}^{d_{\text{feat}}}$为分类器倒数第二层的特征表示，$c_y \in \mathbb{R}^{d_{\text{feat}}}$为目标类别$y$的特征中心。p-reg损失定义为：
...
身份一致性损失确保生成图像的身份特征与标签条件嵌入$e_{\text{id}}$一致，建立类别标签与视觉身份特征的稳定映射关系。本文采用对比学习框架，利用人脸识别模型在单位超球面上的几何性质，显式增强目标身份与负样本身份的角度分离。
...
值得注意的是，分类器引导损失$\mathcal{L}_{\text{cls}}$与特征空间正则化损失$\mathcal{L}_{\text{p-reg}}$作为独立损失项分别进行不确定性加权，两者协同驱动分类器攻击。其中，$\mathcal{L}_{\text{cls}}$直接优化分类器logits使生成图像远离决策边界，$\mathcal{L}_{\text{p-reg}}$通过最小化生成特征与预计算类别中心的距离稳定优化轨迹，防止优化过程中的振荡。这种分离的权重调节机制允许模型根据训练动态自动调配两种互补优化策略的相对重要性。

\subsection{渐进式三阶段训练与标签引导推理}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{images/train_mia.pdf}
    \caption{标签重建人脸方法渐进式三阶段训练流程}
    \label{fig:mia_train}
\end{figure}

第一阶段为图像条件预热。该阶段利用公开人脸数据集（如CelebA、FFHQ）的真实图像，使用预训练ArcFace编码器从图像提取身份嵌入$e_{\text{id}} = E_{\text{id}}(x_{\text{real}})$。标签条件嵌入层$\mathcal{E}_\psi$在此阶段完全不参与训练，目的是让LoRA适配层专注学习如何利用外部提供的身份嵌入进行人脸替换，建立"身份嵌入向量→高质量人脸替换生成"的映射能力，而不是学习如何生成这些嵌入。这种解耦训练策略确保LoRA层能够接受任意符合ArcFace嵌入空间分布的向量，为后续标签嵌入的注入奠定基础。冻结人脸替换模型主干与扩散U-Net，仅训练LoRA参数$\{A_\ell, B_\ell\}$，损失函数为
\begin{equation}\label{eq:mia_stage0_loss}
    \mathcal{L}_{\text{stage0}} = \mathcal{L}_{\text{prior}} + \mathcal{L}_{\text{perc}}.
\end{equation}

该阶段聚焦于扩散先验保真度与感知质量，使LoRA适配层学习如何在保持生成质量的前提下利用外部身份嵌入进行人脸替换。训练持续至生成图像FID稳定在较低水平。
...
其中前项为基础生成质量约束（先验保真度与感知质量），后项为攻击优化相关损失（分类器引导、特征正则化、身份一致性与嵌入正则化）。所有损失项权重通过不确定性参数$\{\sigma_i\}$自动平衡，退火系数$\lambda(t)$仅控制攻击损失的引入速度，不干扰各损失间的相对权重调整：初期$\lambda(t)\approx 0$时，攻击损失几乎不参与训练，模型利用真实图像嵌入专注于学习高质量人脸替换生成；中期$\lambda(t)\approx 0.5$时，攻击损失逐步激活，标签嵌入在分类器引导、特征正则化与身份一致性损失的共同作用下逐步接管身份控制；后期$\lambda(t)\approx 1.0$时，攻击损失完全激活，模型完全适配标签条件生成并优化分类器攻击效果，同时生成质量约束始终存在以防止攻击优化破坏图像真实性。
...
该阶段同时优化攻击有效性与生成质量，各损失权重通过任务不确定性框架自动学习。其中分类器引导损失$\mathcal{L}_{\text{cls}}$与特征空间正则化损失$\mathcal{L}_{\text{p-reg}}$协同驱动分类器攻击，前者最大化目标类别置信度并远离决策边界，后者稳定优化轨迹并约束特征表示向类别原型靠拢。LoRA参数采用较小学习率以避免遗忘已学习的生成能力。训练持续至攻击成功率与生成质量同时达到预期目标。

算法~\ref{alg:mia_progressive}给出了渐进式三阶段训练的统一实现流程。该算法通过阶段标识$s\in\{0,1,2\}$区分不同训练阶段，各阶段在身份嵌入生成方式、损失函数组成及参数更新策略上存在差异。

\begin{algorithm}[!t]
  \caption{渐进式三阶段SwapMIA训练算法}
...
训练完成后，模型可直接用于生成目标类别的攻击样本。推理阶段的核心目标是在保证分类器认同度的前提下，生成具备高感知质量和样本多样性的攻击图像集合。如图~\ref{fig:mia_infer}所示，推理流程由身份嵌入生成、条件人脸替换生成与质量筛选三个步骤组成。

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{images/infer_mia.pdf}
    \vspace{-0.5em}
    \caption{标签重建人脸方法推理流程}
    \label{fig:mia_infer}
\end{figure}
...
该嵌入向量经过三阶段渐进式训练后，已建立类别标签与身份特征空间的稳定映射。归一化后的嵌入$e_{\text{id}}/\|e_{\text{id}}\|_2$位于ArcFace编码器学习的单位超球面上，确保与人脸替换模型的身份控制接口兼容。

其次，为生成具有多样性的攻击样本，需从公开人脸数据集中采样多个源图像作为属性提供者。对于每个源图像$x_{\text{src}}^{(j)}$，结合身份嵌入$e_{\text{id}}$通过微调后的人脸替换模型生成攻击样本：
\begin{equation}\label{eq:mia_inference_generation}
    \hat{x}^{(j)} = G_{\phi+\Delta}(x_{\text{src}}^{(j)}, e_{\text{id}})
\end{equation}

其中$G_{\phi+\Delta}$表示融合LoRA增量$\Delta=\{A_\ell, B_\ell\}$后的人脸替换模型，$\phi$为冻结的预训练主干参数。生成过程遵循扩散模型的标准采样流程：VAE编码器将源图像编码为潜在表示$z_0 = \text{VAE}_{\text{enc}}(x_{\text{src}}^{(j)})$，随后通过DDIM多步去噪采样生成目标潜在表示$\hat{z}_0$，最后VAE解码器输出生成图像$\hat{x}^{(j)} = \text{VAE}_{\text{dec}}(\hat{z}_0)$。身份嵌入$e_{\text{id}}$通过参考注意力机制注入U-Net去噪网络的每个时间步，实现身份控制。

为进一步提升攻击有效性，SwapMIA在推理阶段引入分类器引导采样机制。训练阶段虽已通过分类器损失优化嵌入层与LoRA参数，但推理时的标准采样流程未显式利用分类器信息，导致生成样本可能偏离目标类别的高置信区域。借鉴扩散模型的分类器引导技术\cite{rombach2022high}，SwapMIA在每个去噪时间步$t$引入分类器梯度修正采样轨迹，驱动生成过程持续逼近目标类别。具体而言，在DDIM采样的第$t$步，原始去噪网络预测噪声$\epsilon_\theta(z_t, t, e_{\text{id}})$，通过该预测可估计当前潜在表示对应的干净图像$z_{0|t} = (z_t - \sqrt{1-\bar{\alpha}_t}\epsilon_\theta) / \sqrt{\bar{\alpha}_t}$。将$z_{0|t}$解码为像素空间图像$x_{0|t} = \text{VAE}_{\text{dec}}(z_{0|t})$并输入目标分类器，计算其在目标类别$y_{\text{target}}$的对数概率$\log p_\theta(y_{\text{target}}|x_{0|t})$。对该对数概率关于潜在表示$z_t$求梯度，得到引导方向$\nabla_{z_t} \log p_\theta(y_{\text{target}}|x_{0|t})$，该梯度指示了使分类器置信度增大的潜在空间移动方向。引导后的噪声预测修正为：
...
源图像集合$\{x_{\text{src}}^{(j)}\}_{j=1}^{N_{\text{src}}}$的选择对攻击效果有重要影响。SwapMIA采用多样性驱动的采样策略，从公开数据集中采样若干源图像，确保覆盖不同姿态、表情、年龄、光照等属性维度。可通过预训练的属性估计器评估候选集的属性方差，选择方差较大的子集以最大化生成样本的多样性。该策略继承人脸替换模型的核心优势，即身份与属性解耦，使生成图像在保持目标身份的同时，从源图像继承丰富的外观属性。
...
其中$W_{\text{frozen}}$为冻结的预训练权重，$A, B$为训练得到的LoRA矩阵，$\alpha$为缩放因子，$r$为LoRA秩。合并后模型结构与原始人脸替换模型完全一致，可作为独立的攻击工具部署，无需额外适配层或动态参数注入。


\section{实验验证}[Experimental Validation]
\label{sec:mia_experiments}

本节通过系统实验验证前述提出的基于人脸替换的标签重建人脸方法的有效性。实验在不同目标分类器架构上进行定量对比以评估攻击成功率与生成质量，并通过消融实验量化各关键模块的性能贡献。
...
针对标签重建人脸任务，采用以下指标评估重建图像的攻击有效性与生成质量：

（1）目标准确率与评估准确率。目标准确率衡量生成图像在目标分类器上的Top-1命中率，即分类器预测的Top-1类别与目标标签匹配的比例，直接反映攻击对目标模型的有效性。评估准确率（即交叉评估准确率）使用一个与目标分类器相互独立的人脸识别模型对生成样本进行二次验证，防止生成图像仅对目标分类器过拟合而无法真正代表目标类别的视觉特征，是衡量攻击样本泛化代表性的关键指标。两项指标越高表示攻击越有效且越具代表性。

（2）FID。与模板重建人脸任务相同，用于评估生成图像分布与真实训练数据分布的相似度。

（3）KNN距离（KNN Dist）。基于余弦相似度计算生成图像与目标类别真实图像在ArcFace特征空间中的K近邻平均相似度，反映生成图像与目标类别特征分布的接近程度。该值越高表示生成图像与真实样本在特征空间中的相似性越强。
...
对比的基准方法包括GMI~\cite{zhang2020secret}基于生成对抗网络，FMI~\cite{khosravy2022model}利用特征匹配，PLG-MI~\cite{yuan2023pseudo}采用伪标签引导的条件生成对抗网络，以及Diff-MI~\cite{li2024model}基于扩散模型的方法。

\subsection{基准性能对比}
\label{sec:mia_benchmark}

表~\ref{tab:mia_sota_comparison}对比了SwapMIA与四种现有代表性方法在三种目标分类器架构下的攻击性能。对比方法包括：GMI~\cite{zhang2020secret}基于生成对抗网络，FMI~\cite{khosravy2022model}利用特征匹配，PLG-MI~\cite{yuan2023pseudo}采用伪标签引导，Diff-MI~\cite{li2024model}基于扩散模型。评估指标包括目标准确率（TarAcc）、评估准确率（EvalAcc）、生成保真度（FID）以及KNN距离（KNN Dist）。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccccccccccc}
    ...
    \hline
    SwapMIA & \textbf{94.87\%} & \textbf{83.15\%} & \textbf{23.26} & 0.7165  & \textbf{93.28\%} & \textbf{84.23\%} & \textbf{25.83} & \textbf{0.8534}    & 94.38\% & \textbf{82.94\%} & \textbf{26.91} & \textbf{0.9024} \\
    \hline
  \end{tabular}
  }
\end{table}

在ArcFace目标分类器上，SwapMIA达到94.87\%目标准确率和83.15\%评估准确率，FID为23.26，KNN距离为0.7165。对比Diff-MI的94.18\%目标准确率和75.62\%评估准确率，SwapMIA在评估准确率上提升7.53个百分点，FID降低6.27，表明生成样本不仅能攻击目标模型，更能真正代表目标类别特征，在独立评估模型上保持高识别率。在IR152和Face.evoLVe分类器上，SwapMIA同样在评估准确率和FID上展现一致优势，评估准确率分别达到84.23\%和82.94\%，FID分别为25.83和26.91，均优于所有基准方法。

跨架构性能分析验证了方法的泛化能力。三种分类器采用不同网络架构和训练策略，ArcFace基于角度损失的度量学习，IR152基于改进残差网络，Face.evoLVe使用专门优化的识别架构。在所有架构上，SwapMIA在评估准确率、FID和KNN距离指标上保持显著优势，表明基于REFace的生成先验能有效适应不同特征空间，通过LoRA微调精准拟合不同架构的特征分布。评估准确率在所有架构上均超过82\%，显著高于Diff-MI的75-79\%，证明生成样本具有强泛化能力。虽然在Face.evoLVe分类器上的目标准确率（94.38\%）略低于Diff-MI（95.42\%），但SwapMIA在评估准确率上的显著优势（82.94\% vs. 79.21\%）表明生成样本更能真实代表目标类别特征。SwapMIA仅需微调LoRA参数即可在所有架构上取得稳定高质量重建，验证了标签条件嵌入层设计的有效性和参数高效性。

\subsection{消融实验}[Ablation Study]
\label{sec:mia_ablation}

为量化本章提出的各关键设计模块的贡献，本节在VGGFace2数据集上训练的ArcFace目标分类器上进行系统的消融实验。实验从两个维度评估:渐进式三阶段训练策略的作用，以及各损失项的性能贡献。
\begin{table}[htbp]
  \centering
  \caption{渐进式训练策略消融实验结果}
...
表~\ref{tab:mia_training_ablation}对比不同训练策略对攻击性能的影响。直接使用随机初始化标签嵌入的单阶段训练，目标准确率仅为70.85\%，评估准确率63.74\%。引入图像预热的两阶段训练，通过真实图像建立身份嵌入到人脸替换生成的映射，目标准确率提升至88.92\%，评估准确率78.56\%。采用混合条件过渡的三阶段训练，通过余弦退火实现从图像到标签的平滑模态转换，目标准确率达到94.87\%，评估准确率83.15\%，较单阶段分别提升24.02和19.41个百分点，FID降至23.26，KNN距离达到0.7165，验证了平滑模态迁移对实现高质量标签重建人脸的关键作用。
...
\begin{figure}[htbp]
  ...
  \vspace{-1em}
  \caption{三阶段渐进式训练过程的损失与指标演化曲线}
  \label{fig:mia_training_curves}
\end{figure}
图~\ref{fig:mia_training_curves}展示三阶段训练的损失与指标演化。图中背景色区分训练阶段:阶段1（蓝色）为图像条件预热，阶段2（黄色）为混合条件过渡，阶段3（绿色）为纯标签条件适配。子图(a)展示总损失与部分核心损失的演化趋势，反映整体优化效果;子图(b)呈现全部损失项的独立演化，验证各损失函数的收敛性。子图(c)显示FID和KNN距离两项质量指标的变化，在阶段1图像条件预热阶段，由于仅建立图像到人脸替换的基础映射而未针对目标类别优化，FID维持在较高水平，KNN距离保持在极低值几乎无变化;阶段2引入标签条件后，随着混合系数$\lambda(t)$从0逐渐增长至1，标签条件的贡献逐步增强，两项指标开始显著改善，FID快速下降，KNN距离大幅跃升，充分体现了标签条件引导对目标类别特征匹配的关键作用;阶段3通过LoRA微调进一步优化，两项指标持续提升直至收敛，最终达到表~\ref{tab:mia_training_ablation}所示的三阶段训练结果。子图(d)展示目标准确率和评估准确率的提升过程，呈现与质量指标相似的演化规律。在阶段1，由于尚未建立标签到嵌入的映射，两项准确率保持在极低水平;阶段2引入标签条件后，准确率快速上升;阶段3进一步提升，最终收敛至表~\ref{tab:mia_training_ablation}的三阶段训练策略对应值。子图(e)展示任务不确定性参数$\sigma_i$的动态演化，扩散先验的$\sigma_{\text{prior}}$在阶段1保持较低水平确保生成质量，分类器引导和身份一致性的$\sigma_{\text{cls}}$、$\sigma_{\text{id}}$在阶段2-3显著下降驱动攻击准确率提升;子图(f)展示对应的权重系数$1/(2\sigma_i^2)$的变化，清晰反映各任务重要性的自适应调整过程，该自动权重调整机制使框架无需人工超参数搜索即可在多目标间达到动态均衡。


\begin{table}[htbp]
  \centering
  \caption{标签重建人脸方法损失项组合消融实验}
  ...
  \begin{tabular}{lcccc}
    ...
    + 分类器引导 $\mathcal{L}_{\text{cls}}$ & 77.24\% & 70.38\% & 36.82 & 0.5387 \\
...
表~\ref{tab:mia_loss_ablation}展示不同损失项组合对性能的影响。仅使用扩散先验损失时，缺乏针对目标分类器的优化信号，目标准确率仅为7.85\%。引入分类器引导损失后，目标准确率显著提升至77.24\%。添加身份一致性损失，目标准确率达到92.56\%，评估准确率79.47\%。引入感知质量损失后，目标准确率达到94.87\%，评估准确率83.15\%，FID降至23.26。每增加一个损失项，评估准确率提升幅度均大于目标准确率，特别是身份一致性损失带来显著提升，表明这些损失项不仅提高攻击成功率，更增强生成样本的真实代表性和泛化能力，验证了多目标优化框架的必要性。


\subsection{重建结果与分析}[Visualization Results and Analysis]
\label{sec:mia_visualization}

为直观展示标签重建人脸图像的生成质量与多样性，图~\ref{fig:mia_grid_by_target}以矩阵形式组织生成样本。矩阵第一列为源人脸图像，充当属性提供者，涵盖不同的姿态、表情与光照条件；后续各列对应不同的目标身份类别，列顶部标注目标类别标签编号。沿行方向，同一行的生成图像均以左侧源人脸为属性来源，针对不同目标类别生成对应的攻击样本，可观察到姿态与表情属性被准确保留，而面部轮廓、五官比例、发型等关键身份特征已转换为目标身份；沿列方向，同一列的生成图像虽源于不同源人脸，但在目标身份的视觉特征上保持高度一致，证明标签条件嵌入层能够将离散类别标签映射为具有判别性的连续身份嵌入。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{images/grid_by_target.jpg}
  \caption{标签重建人脸方法针对不同目标类别的重建结果}
  \label{fig:mia_grid_by_target}
\end{figure}

生成图像不仅被目标分类器正确识别为对应类别，同时在视觉质量上呈现良好真实感，包括自然的肤色分布、清晰的五官轮廓以及合理的面部结构比例。利用REFace人脸替换模型先验显著提升生成质量，预训练的人脸替换模型为生成过程提供强大面部几何先验知识，使生成图像在面部结构一致性和光照自然度方面表现优异。标签条件嵌入层通过MLP网络成功实现从离散类别标签到连续身份表示空间的映射，不同类别的生成图像呈现明显身份区分度，验证标签条件嵌入机制的有效性。

上述重建结果验证SwapMIA在保持高攻击准确率的同时能显著提升重建图像的保真度和视觉质量，基于人脸替换模型先验的生成框架与标签条件嵌入机制的有机结合使SwapMIA在攻击有效性与生成真实感之间实现良好平衡，展现出在实际应用场景中的潜在威胁性。

\section{本章小结}[Summary]
\label{sec:mia_summary}

本章提出基于人脸替换的标签重建人脸方法（SwapMIA），利用预训练人脸替换模型的身份属性解耦机制，通过标签条件嵌入层建立离散类别标签与连续身份嵌入空间的映射，采用LoRA微调技术以极少参数开销适配新的嵌入分布，设计渐进式三阶段训练策略实现从图像条件到标签条件的平滑模态迁移，结合多目标自适应优化框架动态平衡扩散先验、分类器攻击、身份一致性与感知质量等目标，并在推理阶段引入分类器引导采样机制提升攻击有效性，实现从类别标签到高保真攻击样本的生成。实验结果表明，SwapMIA在多种目标分类器架构上均表现出优异的攻击准确率与生成保真度，消融实验验证了渐进式训练策略、LoRA微调与任务不确定性加权框架等核心模块的有效性。
% Local Variables:
% TeX-master: "../main"
% TeX-engine: xetex
% End:
