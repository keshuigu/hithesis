...
为直观展示标签重建人脸图像的生成质量与多样性，图~\ref{fig:mia_grid_by_target}以矩阵形式组织生成样本。矩阵第一列为源人脸图像，充当属性提供者，涵盖不同的姿态、表情与光照条件；后续各列对应不同的目标身份类别，列顶部标注目标类别标签编号。沿行方向，同一行的生成图像均以左侧源人脸为属性来源，针对不同目标类别生成对应的攻击样本，可观察到姿态与表情属性被准确保留，而面部轮廓、五官比例、发型等关键身份特征已转换为目标身份；沿列方向，同一列的生成图像虽源于不同源人脸，但在目标身份的视觉特征上保持高度一致，证明标签条件嵌入层能够将离散类别标签映射为具有判别性的连续身份嵌入。这种可视化布局同时验证了方法跨目标类别的攻击能力与跨源人脸的生成多样性。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{images/grid_by_target.jpg}
  \caption{标签重建人脸方法针对不同目标类别的生成样本可视化}
...
上述可视化结果验证SwapMIA在保持高攻击准确率的同时能显著提升重建图像的保真度和视觉质量，基于人脸替换模型先验的生成框架与标签条件嵌入机制的有机结合使SwapMIA在攻击有效性与生成真实感之间实现良好平衡，展现出在实际应用场景中的潜在威胁性。
