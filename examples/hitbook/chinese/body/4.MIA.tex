...
本章提出基于换脸先验迁移的多目标自适应模型反演方法，利用预训练换脸模型的身份属性解耦机制，通过标签条件嵌入层、低秩适应微调、渐进式训练策略与多目标自适应优化框架，实现从类别标签到高保真攻击样本的生成。

方法的核心创新体现在四个方面。首先，标签条件嵌入层通过MLP网络建立离散类别标签与连续身份嵌入空间的映射，为纯标签条件生成奠定基础。其次，低秩适应（LoRA）微调技术通过在关键层引入可训练的低秩增量矩阵，以极少的参数开销适配新的嵌入分布，避免破坏预训练模型的先验知识。再次，渐进式三阶段训练策略通过图像条件预热、混合条件平滑过渡与纯标签条件适配的有序推进，结合余弦退火调度实现模态的稳定迁移。最后，多目标自适应优化框架采用任务不确定性加权机制，在扩散先验保真度、分类引导有效性、身份语义一致性与感知质量之间实现动态平衡，避免人工调参。推理阶段通过源图像采样与样本筛选策略生成多样化攻击结果。
