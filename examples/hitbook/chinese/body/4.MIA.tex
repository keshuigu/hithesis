...
本章首先形式化定义问题与威胁模型，随后阐述换脸先验模型的选择与适配改造、标签条件嵌入层设计、低秩适应微调策略、多目标优化损失函数框架与渐进式三阶段训练策略，最后给出完整的训练算法与分类器引导推理流程。
...
本章提出基于换脸先验迁移的多目标自适应模型反演方法，利用预训练换脸模型的身份属性解耦机制，通过标签条件嵌入层、低秩适应微调、渐进式训练策略、多目标自适应优化框架与分类器引导推理机制，实现从类别标签到高保真攻击样本的生成。

方法的核心创新体现在五个方面。首先，标签条件嵌入层通过MLP网络建立离散类别标签与连续身份嵌入空间的映射，结合内存库机制实现对比学习驱动的类间分离，为纯标签条件生成奠定基础。其次，低秩适应（LoRA）微调技术通过在参考注意力层、U-Net注意力层与残差卷积层引入可训练的低秩增量矩阵，以极少的参数开销适配新的嵌入分布，避免破坏预训练模型的先验知识。再次，渐进式三阶段训练策略通过图像条件预热、混合条件平滑过渡与纯标签条件适配的有序推进，结合余弦退火调度实现模态的稳定迁移。第四，多目标自适应优化框架采用任务不确定性加权机制，在扩散先验保真度、分类器攻击有效性（top-k max-margin与p-reg）、身份语义一致性、感知质量与正则化约束之间实现动态平衡，避免人工调参。最后，推理阶段引入分类器引导采样机制，通过稀疏的梯度修正驱动生成轨迹持续逼近目标类别的高置信区域，结合源图像多样性采样与质量筛选策略生成多样化的高置信度攻击样本。
