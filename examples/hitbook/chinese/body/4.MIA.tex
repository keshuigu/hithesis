...
标签重建人脸方法针对基于分类的端到端人脸识别模型。设分类模型为 $F_\theta: \mathbb{R}^{H \times W \times C} \to \mathbb{R}^N$，其中 $N$ 为训练类别数，$\theta$ 为模型参数，模型输出各身份类别的置信度分布。本章采用白盒威胁模型：攻击者仅持有目标身份对应的离散类别索引 $y \in \{1,2,\ldots,N\}$，不具备任何真实目标图像或特征嵌入向量；攻击者掌握目标分类器的完整架构与参数，可执行前向推理与反向梯度计算（即 $\nabla_x F_\theta(x)$ 可获取）；攻击者可使用与目标系统训练集无重叠的公开人脸数据集（如VGGFace2、CelebA）训练生成模型，并可访问公开预训练生成先验（如扩散式人脸替换骨干网络）进行迁移适配，但无法访问目标系统的原始训练样本。该设定代表仅凭模型访问权限与公开资源的最强攻击上界，对应模型开源、接口暴露或逆向工程成功的现实场景。

攻击目标是生成图像 $\hat{x}$，使其同时满足分类器认同与视觉真实性两项核心约束：前者要求 $F_\theta(\hat{x})_y \geq \tau$，其中 $\tau$ 为置信度阈值，生成图像经分类器强烈认定为目标身份；后者要求 $\hat{x} \in \mathcal{M}_{\text{natural}}$，即在感知质量上与真实人脸无明显差异。与模板重建人脸不同，MIA的优化目标是最大化分类器置信度而非最小化特征距离，攻击过程须直接利用 $\nabla_x F_\theta(x)_y$ 引导生成。
...
式中，第一项驱动生成图像被分类器识别为目标身份；$\mathcal{L}_{perc}$ 为感知质量约束，常采用LPIPS\cite{zhang2018unreasonable}等基于深度特征的感知相似度度量；$R(\cdot)$ 为正则化项，防止生成图像偏离自然图像流形；$\lambda_{perc}$、$\lambda_{reg}$ 为权衡系数。攻击成功分三级评估：强成功要求 $F_\theta(\hat{x})_y \geq \tau$ 且生成图像可直接被系统接受；弱成功要求目标类别输出值显著高于其他类别，表明目标身份的判别特征已被部分恢复；视觉成功要求生成图像具有自然的人脸外观并通过人类视觉检验。此外，对同一目标类别生成的多张图像还应在姿态、表情、光照等属性维度上呈现合理变化，以评估攻击对目标身份分布的覆盖程度。
...
（1）目标准确率（TarAcc）。目标准确率衡量生成图像在目标分类器上的Top-1命中率，即分类器预测的Top-1类别与目标标签匹配的比例，直接反映攻击样本是否骗过目标模型，反映目标模型的直接泄露风险。

（2）评估准确率（EvalAcc）。评估准确率使用一个与目标分类器相互独立的人脸识别模型对生成样本进行二次验证，防止生成图像仅对目标分类器过拟合而无法真正代表目标类别的视觉特征。评估准确率反映攻击样本是否包含可迁移的身份信息，在独立模型上也能识别，是衡量攻击样本泛化代表性的关键指标。

（3）FID。与模板重建人脸任务相同，用于评估生成图像分布与真实训练数据分布的相似度。

（4）KNN距离（KNN Dist）。基于余弦相似度计算生成图像与目标类别真实图像在ArcFace特征空间中的K近邻平均相似度，反映生成图像与目标类别特征分布的接近程度。该值越高表示生成图像与真实样本在特征空间中的相似性越强，更接近目标身份核心判别特征，代表隐私信息恢复更深、更精确。
