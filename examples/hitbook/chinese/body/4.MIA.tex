...
模型反演攻击针对基于分类的端到端人脸识别系统。这类系统将人脸识别建模为多分类问题，设分类模型为 $F_\theta: \mathbb{R}^{H \times W \times C} \to \mathbb{R}^N$，其中 $N$ 为训练类别数，$\theta$ 为模型参数。模型通过softmax层输出各类别的概率分布，在训练过程中学习将输入图像映射到对应身份类别。给定目标类别 $y \in \{1, 2, \ldots, N\}$，攻击者的目标是生成图像 $\hat{x}$，使其同时满足两个核心特性。

首先是分类器认同，即生成图像应使分类器在目标类别上产生高置信度输出。形式化地，要求 $F_\theta(\hat{x})$ 在目标类别 $y$ 上的输出值显著高于其他类别，即 $F_\theta(\hat{x})_y > F_\theta(\hat{x})_j, \forall j \neq y$。在实际攻击场景中，通常要求满足 $F_\theta(\hat{x})_y \geq \tau$，其中 $\tau$ 为置信度阈值，在人脸识别系统中常设定为0.5到0.9之间。高置信度输出表明生成图像被分类器强烈认定为目标身份，意味着攻击成功恢复了训练数据中该类别的关键特征。其次是视觉真实性，即生成图像 $\hat{x}$ 应在感知质量上与真实人脸图像无明显差异，具有自然的面部特征和纹理细节，需要属于自然人脸图像流形 $\mathcal{M}_{\text{natural}}$。

形式化地，模型反演攻击问题可建模为如下优化问题：
\begin{equation}\label{eq:mia_optimization}
\hat{x} = \arg\max_x \; F_\theta(x)_y + \lambda_{perc}\mathcal{L}_{perc}(x) - \lambda_{reg}R(x)
\end{equation}

其中第一项为目标类别的分类器输出，通过最大化该项驱动生成图像被识别为目标身份；第二项 $\mathcal{L}_{perc}$ 为感知质量约束，常采用LPIPS等学习的感知度量，确保生成图像具有自然的视觉外观；第三项 $R(\cdot)$ 为正则化项，防止生成图像偏离自然图像流形；$\lambda_{perc}$ 和 $\lambda_{reg}$ 为权衡系数。与模板逆向攻击不同，模型反演攻击的优化目标是最大化分类器输出而非最小化特征距离，这使得攻击过程需要直接利用分类器的梯度信息 $\nabla_x F_\theta(x)_y$ 引导生成过程。
...
为直观展示模型反演方法的生成质量与多样性，图~\ref{fig:mia_grid_by_target}展示了针对不同目标类别的模型反演结果。该图采用矩阵形式组织生成样本，其中每行对应一个源人脸图像，每列对应一个目标身份类别，列顶部的数字标注为目标类别标签编号。具体而言，每行展示了使用同一源人脸作为属性提供者时，通过标签条件嵌入层生成的针对不同目标类别的换脸结果；每列展示了针对同一目标类别标签，使用不同源人脸生成的攻击样本。这种可视化布局能够同时验证方法在固定源人脸条件下跨多个目标类别的攻击能力和生成多样性。

从图中可观察到以下关键特征：同一行的生成图像继承了源人脸的姿态和表情属性，但在关键身份特征如面部轮廓、五官比例、发型等方面已转换为目标身份特征；同一列的生成图像虽源于不同源人脸，但在目标身份的视觉特征上保持高度一致，证明标签条件嵌入层成功学习了目标类别的身份表征，能够将离散的类别标签映射为具有判别性的连续身份嵌入。不同源人脸展现丰富的姿态变化、表情变化和光照条件，验证方法在不同源人脸条件下的鲁棒性。源人脸的姿态和表情属性被准确保留并迁移至目标身份，验证基于REFace的换脸先验能为模型反演任务提供强大结构引导，显著提升生成质量。
