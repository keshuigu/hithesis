...
\chapter{基于人脸替换模型先验迁移的标签逆向重建人脸方法}[Face Image Inverse Reconstruction from Class Labels via Face Replacement Model Prior Migration]\label{chap:MIA}

\section{引言}[Introduction]

模型逆向攻击旨在从训练完成的分类模型中重建其训练数据的敏感特征。传统基于梯度优化的方法直接在像素空间进行优化，面临生成质量低下、优化效率不足等问题。近年来引入生成式先验可提升攻击效果，但现有工作在身份控制精度与生成保真度的平衡上仍存在不足。

本章将扩散人脸替换模型应用于从类别标签逆向重建人脸图像的任务。该模型具有显式的身份属性解耦机制，但其标准推理流程依赖真实目标图像，而任务场景中仅拥有类别标签。本章设计标签条件嵌入层、低秩适应微调、渐进式训练策略、多目标优化框架与分类器引导推理机制，将离散标签映射为连续身份嵌入，实现从类别标签到高保真攻击样本的生成。

本章首先形式化定义问题与威胁模型，随后阐述人脸替换先验模型的选择与适配改造、标签条件嵌入层设计、低秩适应微调策略、多目标优化损失函数框架与渐进式三阶段训练策略，最后给出完整的训练算法与分类器引导推理流程。基于上述方法，本章在VGGFace2、CelebA等标准数据集上进行系统实验，针对ArcFace、IR152与Face.evoLVe等不同架构的目标分类器评估攻击性能，通过与现有代表性方法的对比实验验证本章方法在攻击成功率与生成保真度上的优势，并通过消融实验量化各关键设计模块的性能贡献。
...
首先是分类器认同，即生成图像应使分类器在目标类别上产生高置信度输出。形式化地，要求 $F_\theta(\hat{x})$ 在目标类别 $y$ 上的输出值显著高于其他类别，即 $F_\theta(\hat{x})_y > F_\theta(\hat{x})_j, \forall j \neq y$。在实际攻击场景中，通常要求满足 $F_\theta(\hat{x})_y \geq \tau$，其中 $\tau$ 为置信度阈值，在人脸识别模型中常设定为0.5到0.9之间。高置信度输出表明生成图像被分类器强烈认定为目标身份，意味着攻击成功恢复了训练数据中该类别的关键特征。其次是视觉真实性，即生成图像 $\hat{x}$ 应在感知质量上与真实人脸图像无明显差异，具有自然的面部特征和纹理细节，需要属于自然人脸图像流形 $\mathcal{M}_{\text{natural}}$。
...
其中第一项为目标类别的分类器输出，通过最大化该项驱动生成图像被识别为目标身份；第二项 $\mathcal{L}_{perc}$ 为感知质量约束，常采用LPIPS等学习的感知度量，确保生成图像具有自然的视觉外观；第三项 $R(\cdot)$ 为正则化项，防止生成图像偏离自然图像流形；$\lambda_{perc}$ 和 $\lambda_{reg}$ 为权衡系数。与模板逆向攻击不同，模型逆向攻击的优化目标是最大化分类器输出而非最小化特征距离，这使得攻击过程需要直接利用分类器的梯度信息 $\nabla_x F_\theta(x)_y$ 引导生成过程。
...
人脸替换模型作为本章方法的核心生成先验，需同时满足高质量身份迁移、显式身份-属性解耦与端到端可微分三项基本要求。相比传统的类条件生成先验，人脸替换模型在模型逆向攻击任务中具有三方面本质优势。
...
本章方法的整体架构如图~\ref{fig:mia_architecture}所示，由标签嵌入层$\mathcal{E}_\psi$、人脸替换生成模型（VAE编码器、U-Net+LoRA、VAE解码器）、下游评估模块（分类器$F_\theta$和ArcFace编码器$E_{\text{id}}$）以及六个损失函数组成。
...
本节详细阐述基于人脸替换模型先验迁移的标签逆向重建人脸方法的训练与推理流程。首先介绍渐进式三阶段训练策略，通过图像条件预热、混合条件过渡和纯标签条件适配实现从图像条件到标签条件的平滑模态转换；随后描述整体架构的前向传播与反向传播流程，明确各模块的数据流向与梯度更新机制；最后给出完整的训练算法与推理策略，为方法的实现与复现提供系统化指导。
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{images/train_mia.png}
    \vspace{-1em}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{images/infer_mia.png}
    \vspace{-1em}
    \caption{模型逆向攻击推理流程}
    \label{fig:mia_infer}
\end{figure}
...
源图像集合$\{x_{\text{src}}^{(j)}\}_{j=1}^{N_{\text{src}}}$的选择对攻击效果有重要影响。本方法采用多样性驱动的采样策略，从公开数据集中采样若干源图像，确保覆盖不同姿态、表情、年龄、光照等属性维度。可通过预训练的属性估计器评估候选集的属性方差，选择方差较大的子集以最大化生成样本的多样性。该策略继承人脸替换模型的核心优势，即身份与属性解耦，使生成图像在保持目标身份的同时，从源图像继承丰富的外观属性。
...
本节通过系统实验验证前述提出的基于人脸替换模型先验迁移的标签逆向重建人脸方法的有效性。实验在不同目标分类器架构上进行定量对比以评估攻击成功率与生成质量，并通过消融实验量化各关键模块的性能贡献。
...
对比的基准方法包括GMI~\cite{carlini2019secret}基于生成对抗网络,FMI~\cite{khosravy2022model}利用特征匹配,PLG-MI~\cite{yuan2023pseudo}采用伪标签引导的条件生成对抗网络,以及Diff-MI~\cite{li2024model}基于扩散模型的方法。

\subsection{基准性能对比}
\label{sec:mia_benchmark}

表~\ref{tab:mia_sota_comparison}对比了本文方法与四种现有代表性方法在三种目标分类器架构下的攻击性能。对比方法包括：GMI~\cite{carlini2019secret}基于生成对抗网络，FMI~\cite{khosravy2022model}利用特征匹配，PLG-MI~\cite{yuan2023pseudo}采用伪标签引导，Diff-MI~\cite{li2024model}基于扩散模型。评估指标包括目标准确率（TarAcc）、评估准确率（EvalAcc）、生成保真度（FID）以及KNN距离（KNN Dist）。
...
为直观展示逆向重建人脸的图像的生成质量与多样性，图~\ref{fig:mia_grid_by_target}展示了针对不同目标类别的逆向重建结果结果。该图采用矩阵形式组织生成样本，其中每行对应一个源人脸图像，每列对应一个目标身份类别，列顶部的数字标注为目标类别标签编号。具体而言，每行展示了使用同一源人脸作为属性提供者时，通过标签条件嵌入层生成的针对不同目标类别的人脸替换结果；每列展示了针对同一目标类别标签，使用不同源人脸生成的攻击样本。这种可视化布局能够同时验证方法在固定源人脸条件下跨多个目标类别的攻击能力和生成多样性。

从图中可观察到以下关键特征：同一行的生成图像继承了源人脸的姿态和表情属性，但在关键身份特征如面部轮廓、五官比例、发型等方面已转换为目标身份特征；同一列的生成图像虽源于不同源人脸，但在目标身份的视觉特征上保持高度一致，证明标签条件嵌入层成功学习了目标类别的身份表征，能够将离散的类别标签映射为具有判别性的连续身份嵌入。不同源人脸展现丰富的姿态变化、表情变化和光照条件，验证方法在不同源人脸条件下的鲁棒性。源人脸的姿态和表情属性被准确保留并迁移至目标身份，验证基于REFace的人脸替换模型先验能为逆向重建任务提供强大结构引导，显著提升生成质量。
...
上述可视化结果验证第四章提出的基于人脸替换模型先验迁移的标签逆向重建人脸方法在保持高攻击准确率的同时能显著提升重建图像的保真度和视觉质量,基于人脸替换模型先验的生成框架与标签条件嵌入机制的有机结合使方法在攻击有效性与生成真实感之间实现良好平衡,展现出在实际应用场景中的潜在威胁性。

\section{本章小结}[Summary]
\label{sec:mia_summary}

本章提出基于人脸替换模型先验迁移的标签逆向重建人脸方法，利用预训练人脸替换模型的身份属性解耦机制，通过标签条件嵌入层建立离散类别标签与连续身份嵌入空间的映射，采用低秩适应（LoRA）微调技术以极少参数开销适配新的嵌入分布，设计渐进式三阶段训练策略实现从图像条件到标签条件的平滑模态迁移，结合多目标自适应优化框架动态平衡扩散先验、分类器攻击、身份一致性与感知质量等目标，并在推理阶段引入分类器引导采样机制提升攻击有效性，实现从类别标签到高保真攻击样本的生成。实验结果表明，该方法在多种目标分类器架构上均表现出优异的攻击准确率与生成保真度，消融实验验证了渐进式训练策略、LoRA微调与任务不确定性加权框架等核心模块的有效性。
% Local Variables:
% TeX-master: "../main"
% TeX-engine: xetex
% End:
