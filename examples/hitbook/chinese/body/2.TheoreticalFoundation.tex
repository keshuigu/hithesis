...
第~\ref{sec:face_recognition}~节建立了深度人脸识别系统的形式化框架。通过阐述特征提取网络、嵌入归一化与相似度度量等核心组件，建立了从原始图像到固定维度嵌入向量的完整映射。度量学习方法的分析揭示了如何通过对比损失、三元组损失与角度边距损失在嵌入空间中构造判别性表示，使同一身份样本聚集、不同身份样本分离。该节建立的评估指标体系将直接用于衡量重建图像的身份保真度与攻击成功率，为定义模板逆向攻击的目标函数提供了形式化框架。

第~\ref{sec:diffusion_models}~节系统推导了扩散概率模型的数学原理。从前向扩散的马尔可夫链表述到反向去噪的变分下界优化，建立了完整的理论框架。通过推导后验分布、重参数化技巧与噪声预测目标，揭示了得分函数估计的内在联系。条件生成机制介绍了分类器引导、无分类器引导与基于能量的引导三种技术路径，为在给定人脸嵌入约束下生成目标图像提供了直接方法。DDIM等高效采样策略在保证生成质量的同时显著降低计算开销，使得模板逆向重建在效率与实用性之间取得平衡。

第~\ref{sec:peft_lora}~节阐述了低秩适配技术的理论与实践。通过推导低秩增量 $\Delta W = BA$ 的数学原理，分析了LoRA在参数效率、计算复杂度与存储开销方面的优势。该技术通过在冻结预训练权重的前提下引入可训练增量，实现了参数效率与模型性能的优异平衡，仅需全参数微调成本的1\%至5\%即可完成任务适配。应用位置选择、权重合并与模块化部署等工程策略的讨论，为在有限计算资源下针对多个目标模板进行个性化微调提供了高效技术路径，使得大规模模板重建实验的开展成为可能。

本章在人脸识别的嵌入表示、扩散模型的条件生成、参数高效微调三个层面形成了完整的技术链条，为后续章节的攻击方法设计与实验评估奠定了坚实基础。
