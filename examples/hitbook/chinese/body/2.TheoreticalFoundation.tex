...
本章为后续提出的模板逆向攻击方法与模型反演攻击方法建立理论基础，系统阐述三个核心概念：（1）人脸识别系统——详述系统架构、训练范式、评估标准及模板安全性问题，为定义攻击目标函数与评估指标提供概念框架；（2）扩散概率模型——推导前向扩散、反向去噪、变分下界优化与条件生成机制，并介绍高效采样策略，为构建基于扩散模型的逆向重建攻击方法提供数学工具与算法基础；（3）低秩适配技术——阐述低秩适配的理论与工程实践，为构建模型反演攻击方法的生成模型微调提供高效的技术路径。这三个方面共同构成了本研究从目标定义、生成建模到高效优化的完整理论框架。
...
EDM\cite{karrasEluvidatingDiffusionModels2022}通过系统性分析扩散模型的设计空间，在噪声调度、网络预处理与采样策略等多个方面提出了改进方案。相较于传统DDPM与DDIM，EDM具有以下显著优势：（1）通过重新参数化去噪网络的输入与输出，消除了训练过程中的数值不稳定性；（2）提出了更优的噪声调度方案，确保在不同噪声水平下模型预测误差分布更加均衡；（3）通过统一的采样器设计，在保持生成质量的同时，显著降低了所需的函数评估次数；（4）为条件引导提供了更加稳健的梯度传播路径，使得在高引导强度下仍能保持生成样本的多样性与真实性。
...
第~\ref{sec:face_recognition}~节建立了深度人脸识别系统的形式化框架，阐述了特征提取网络、嵌入归一化与相似度度量等核心组件，分析了度量学习方法在嵌入空间中构造判别性表示的机制，建立了评估指标体系为定义逆向攻击的目标函数提供了形式化基础。第~\ref{sec:diffusion_models}~节系统推导了扩散概率模型的数学原理，从前向扩散的马尔可夫链到反向去噪的变分下界优化，阐述了后验分布、重参数化技巧、噪声预测目标与得分函数估计的内在联系，介绍了条件生成机制与DDIM等高效采样策略。第~\ref{sec:peft_lora}~节阐述了低秩适配技术的理论与实践，分析了LoRA在参数效率、计算复杂度与存储开销方面的优势，介绍了权重合并与模块化部署等工程策略。
