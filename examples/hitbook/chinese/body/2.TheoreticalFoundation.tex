...
\section{引言}
\section{人脸识别模型}
本节对现代深度人脸识别（Face Recognition, FR）系统的核心组成、常用训练范式与评估协议作简明且严谨的阐述，以为第3章提出的嵌入一致性约束、模板逆向重建方法及后续实验评价提供必要的理论与工程依据。

（一）系统组成与表示形式
\newline
典型的人脸识别系统由三个主要模块构成：前处理（对齐、裁剪与归一化）、特征提取网络 $F(\cdot;\theta)$（例如卷积神经网络或 Transformer 骨干）以及用于比较与判决的匹配策略（相似度度量与阈值决策）。对输入图像 $x\in\mathcal{X}$，模型输出一个实值嵌入向量：
\begin{equation}
      f = F(x;\theta)\in\mathbb{R}^d,
\end{equation}
通常对该向量进行归一化（例如 $\tilde f = f/\|f\|_2$），并采用余弦相似度或欧氏距离作为匹配度量：
\begin{equation}
      \mathrm{sim}(f_1,f_2) = \frac{f_1\cdot f_2}{\|f_1\|_2\|f_2\|_2}.
\end{equation}

（二）训练目标：度量学习与判别性损失
\newline
主流方法以度量学习为核心，目标是在嵌入空间中使同一身份样本彼此靠近、不同身份样本彼此分离。常见损失包括对比损失（contrastive loss）、三元组损失（triplet loss）以及在归一化球面上施加角度/边距约束的分类型损失（例如 ArcFace、AM‑softmax 等）。后者通过在 softmax 分类器中引入角度边距，有效构造判别性的球面嵌入，从而提升识别性能与泛化能力。

（三）常用网络结构与工程要点
\begin{itemize}
  \item 骨干网络：常见选型包括 ResNet 系列、MobileNet、Swin/Transformer 等。在追求高性能的场景下，通常采用较深的 ResNet（如 ResNet‑100）或将骨干与 attention 机制结合的混合结构。
  \item 预处理与归一化：经常采用基于人脸关键点（5 点或 68 点）的对齐方法，裁剪到固定分辨率（如 $112\times112$ 或 $224\times224$），并对输入像素进行合适的归一化（例如缩放到 $[-1,1]$ 或基于数据集的均值-方差标准化）。
  \item 嵌入维度与存储：嵌入维度 $d$ 常取 128、256 或 512，具体选择依赖模型容量与应用需求。系统中保存的模板通常为归一化的浮点向量或经过量化的紧凑表示，存储格式与标准化方式会影响后续的可逆性与安全性。
\end{itemize}

（四）评估任务与指标
\newline
人脸识别的评估常分为验证（verification）与识别/检索（identification/retrieval）两类任务：
\begin{itemize}
  \item 验证：判断两幅图像是否属于同一人，常用指标为 TAR@FAR（在指定假接受率 FAR 下的真接受率）、ROC 曲线及其面积（AUC）等；
  \item 识别/检索：在给定的 gallery 中对 probe 进行检索，常关注 top‑k 命中率（尤其 top‑1）。
\end{itemize}

（五）模板安全性与研究相关性
\newline
嵌入（模板）既用于在线比对，也可能长期保存于数据库中，因此属于敏感信息。模板的存储格式（浮点或量化）、归一化策略和是否附带元数据均会影响从模板反推原始生物特征的难易程度。为此，已有研究提出可撤销生物识别、加密模板和限制可逆映射等保护方案。本章所述的嵌入表示、训练与评估协议将直接用于第3章所提出的嵌入一致性损失、模板逆向攻击判据（如式~\eqref{eq:attack_goal}）以及后续的实验评价（包括 TAR@FAR、top‑k、LPIPS、FID 等）。

（六）小结
\newline
本节所述的表示形式、训练范式与评估指标为后续章节提供了必要的概念与评价标准，便于在设计攻击/防御方法及实验对比时保持定义与度量的一致性。


\section{扩散模型基础}

本节首先对扩散生成模型给出概述性说明，为后文的严格推导与算法实现提供必要的理论与符号基础。扩散生成模型通过构建前向噪声过程，将数据分布逐步扰动为近似高斯噪声；随后学习参数化的逆向过程，以噪声尺度为轴逐步去噪恢复样本。理论上，该类模型可由两种互补视角来刻画：一类基于离散时间的变分下界（DDPM/ELBO）表述，另一类基于连续时间的得分函数与随机微分方程（score‑based SDE）表述。两种表述在数学上具有对应关系，其共同目标是在不同噪声尺度下对得分函数或去噪器进行有效估计，从而为生成过程提供正确的梯度信息。

在工程实现层面，为提高生成效率与视觉质量常采用若干改进策略：一方面在潜空间（latent）而非像素空间进行建模（例如 LDM），可显著降低计算与存储开销；另一方面通过设计合理的噪声调度和高效采样器（例如 EDM）以减少采样步数并改善图像质量。为便于后文表述，本文在后续章节中采用如下记号约定：$x_0$ 表示真实样本，$x_t$ 表示时刻 $t$ 的噪声变量；$\epsilon_\theta$ 或 $s_\theta$ 表示参数化的噪声/得分预测网络；$\alpha_t,\beta_t,\overline{\alpha}_t,\sigma_t$ 等为噪声调度相关的标量参数。下文将在此记号体系下给出精确的公式与推导。
...
（二）扩散生成模型的数学与实现要点。系统性地回顾了离散时间 DDPM 的变分表述与基于得分函数的连续时间 SDE 表述，推导了噪声预测目标与得分函数之间的等价性（式~\eqref{eqn-5}、式~\eqn-26 等），并说明了条件化采样（分类器引导与 classifier‑free guidance）与基于能量的引导方法之间的关系。对于工程实现，讨论了在潜空间建模（LDM）、噪声调度与高效采样器（如 EDM/二阶采样器）上的折中与优化要点。
