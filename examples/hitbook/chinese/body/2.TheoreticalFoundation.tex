...
深度学习技术的蓬勃发展使得人脸识别系统在近十年内取得了突破性进展\cite{he2016deep,kingma2014adam}，已广泛应用于身份验证、访问控制、安全监控等关键领域。现代人脸识别系统可采用两种主要架构实现——基于模板匹配的检索型架构和基于分类的端到端架构（详见第\ref{chap:introduction}章第\ref{sec:background}节的架构分析）。无论采用哪种架构，系统的处理流程均可抽象为一个端到端的映射，由以下三个核心模块构成：
