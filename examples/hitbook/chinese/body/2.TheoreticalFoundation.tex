...
第~\ref{sec:face_recognition}~节建立了深度人脸识别系统的形式化框架。通过阐述特征提取网络、嵌入归一化与相似度度量等核心组件，建立了从原始图像到固定维度嵌入向量的完整映射。度量学习方法的分析揭示了如何通过对比损失、三元组损失与角度边距损失在嵌入空间中构造判别性表示。该节建立的评估指标体系将直接用于衡量重建图像的身份保真度与攻击成功率，为定义逆向攻击的目标函数提供了形式化框架。

第~\ref{sec:diffusion_models}~节系统推导了扩散概率模型的数学原理。从前向扩散的马尔可夫链表述到反向去噪的变分下界优化，建立了完整的理论框架。通过推导后验分布、重参数化技巧与噪声预测目标，揭示了得分函数估计的内在联系。条件生成机制为在给定人脸嵌入约束下生成目标图像提供了技术路径，DDIM等高效采样策略在保证生成质量的同时显著降低计算开销。

第~\ref{sec:peft_lora}~节阐述了低秩适配技术的理论与实践。通过推导低秩增量的数学原理，分析了LoRA在参数效率、计算复杂度与存储开销方面的优势。该技术仅需全参数微调成本的1\%至5\%即可完成任务适配，权重合并与模块化部署等工程策略为在有限计算资源下进行个性化微调提供了高效技术路径。
