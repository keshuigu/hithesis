% !Mode:: "TeX:UTF-8"
\chapter{相关理论及技术基础}[Theoretical Foundation]\label{chap:theory}
\section{引言}\label{sec:theory_intro}

生物特征识别系统已广泛应用于身份验证、访问控制、金融支付等关键领域，通常将用户的生物特征转换为固定维度的嵌入向量形成模板，并通过相似度计算实现身份匹配。与传统认证机制相比，生物特征认证具有便捷性与难以伪造性等优势，但随着深度学习技术的深入应用，模板泄露导致的隐私风险显著增加：与传统口令凭证不同，人脸等生物特征具有唯一性与不可撤销性——受害者无法像修改密码一样更换自身的生物特征，因而一旦从泄露模板重建出原始人脸图像，损失便无法弥补；随着生成式模型技术的发展，此类重建已成为现实威胁，攻击者可利用重建图像进行身份伪造或隐私侵犯。

本章为后续各章节提出的图像重建方法建立理论基础，系统阐述三个核心概念：（1）人脸识别模型——详述系统架构、训练范式及嵌入表示安全性问题，为定义图像重建目标函数与评估指标提供概念框架；（2）扩散概率模型——推导前向扩散、反向去噪、变分下界优化与条件生成机制，并介绍高效采样策略，为构建基于扩散模型的图像重建方法提供数学工具与算法基础；（3）低秩适配微调——阐述低秩适配的理论与工程实践，为生成模型的参数高效微调提供方法支撑。这三个方面共同构成了本研究从目标定义、生成建模到高效优化的完整理论框架。

本章组织结构如下：第~\ref{sec:face_recognition}~节介绍人脸识别模型的核心组件与训练方法，建立特征嵌入表示与评估指标体系；第~\ref{sec:diffusion_models}~节推导扩散概率模型的理论框架，阐述前向扩散过程、反向去噪过程、训练优化方法与条件生成机制；第~\ref{sec:peft_lora}~节介绍低秩适配微调的数学原理与工程实现，分析其在参数高效微调中的优势。

\section{人脸识别模型}\label{sec:face_recognition}

\subsection{系统架构与特征表示}

现代深度人脸识别模型\cite{he2016deep,kingma2014adam}可采用两种主要架构实现——基于模板匹配的检索型架构和基于分类的端到端架构。对上述两种架构进行形式化抽象，可归纳出统一的模块化结构，其核心功能组件如下：

（1）预处理模块。该模块负责将原始输入图像转换为适合网络输入的标准化表示。具体包括：（1）人脸检测与对齐：采用多任务级联卷积网络或基于锚框的检测器定位人脸区域，并通过人脸关键点进行仿射变换，将人脸对齐到标准姿态；（2）裁剪与尺度归一化：将对齐后的人脸裁剪为固定分辨率，常用 $112\times112$ 或 $224\times224$ 像素，以适配后续网络的输入要求；（3）像素值归一化：对输入像素进行归一化处理，例如缩放到 $[-1,1]$ 区间或基于训练集统计量进行零均值、单位方差标准化，以改善网络训练的收敛性与鲁棒性。

（2）特征提取模块。该模块是识别模型的核心，通常采用深度卷积神经网络或 Transformer 等神经网络架构实现。给定预处理后的输入图像 $x\in\mathcal{X}$，特征提取网络 $F(\cdot;\theta)$ 输出一个固定维度的实值嵌入向量：
...
（3）匹配与决策模块。在推理阶段，系统通过计算查询图像嵌入 $\tilde{f}_1$ 与参考模板嵌入 $\tilde{f}_2$ 之间的相似度来判断是否属于同一身份。常用的相似度度量为余弦相似度或欧式距离：
\begin{align}
...
人脸识别模型的性能在很大程度上取决于神经网络架构的表达能力。常用的神经网络架构包括以下几类：（1）残差网络（ResNet）\cite{he2016deep}及其改进变体：通过引入残差连接缓解了深度网络的梯度消失问题，是人脸识别中最广泛使用的网络架构之一；在此基础上，面向人脸识别任务设计的改进残差网络（iResNet）\cite{deng2019arcface}在残差块中引入批归一化与PReLU激活的组合，并移除最后全连接层后的批归一化，使特征在超球面上分布更为均匀，显著提升了身份判别能力；（2）在移动端或资源受限场景下，常采用 MobileNet\cite{howard2017mobilenets}、EfficientNet\cite{tan2019efficientnet} 等轻量级架构，以在保持较高精度的同时降低计算与存储开销；（3）近年来，基于自注意力机制的 Vision Transformer\cite{dosovitskiy2020image}及其变体在人脸识别中也展现出了优异性能，例如 Swin Transformer\cite{liu2021swin} 通过层次化移位窗口注意力机制兼顾了局部与全局建模能力，在大规模数据集上的泛化能力更强；同时，围绕视觉Transformer的人脸识别改进研究也从注意力结构优化角度验证了Transformer路线在识别任务中的可行性\cite{JSGG202308011}。
...
深度人脸识别模型在实际部署中通常面向两类任务。人脸验证是一对一的身份比对任务：系统分别提取两幅图像的嵌入向量 $\tilde{f}_1$ 与 $\tilde{f}_2$，计算二者的余弦相似度并与预设阈值 $\tau$ 进行比较，以判定是否属于同一身份。人脸识别则是一对多的检索任务：系统将查询图像的嵌入向量与注册身份库中的所有模板依次计算相似度，以最高相似度所对应的身份作为识别结果。上述两类任务均以归一化特征嵌入向量为核心表示，对应本节开头所述的检索型架构；而基于分类的端到端架构则直接输出类别标签，同样广泛应用于封闭集人脸识别场景。

\subsection{模型训练方法}

深度人脸识别模型的训练目标是学习一个嵌入空间，使得同一身份的样本在该空间中距离较近，不同身份的样本相互远离。这一目标通常通过度量学习实现\cite{wang2014learning}，主流方法可归纳为以下几类：
...
扩散概率模型作为一类新兴的深度生成模型,近年来在图像生成、语音合成、分子设计等多个领域展现出了卓越的性能\cite{ho2020denoising,song2020score}。与生成对抗网络\cite{goodfellow2020generative}和变分自编码器\cite{kingma2013auto}相比,扩散模型通过构建可逆的马尔可夫链,将数据分布逐步转化为简单的高斯分布,再通过学习过程反向实现数据生成,具有训练稳定、生成质量高、模式覆盖全面等优势。本节基于离散时间的去噪扩散概率模型（Denoising Diffusion Probabilistic Models，DDPM）框架,系统推导扩散模型的理论基础、训练优化方法、高效采样策略与条件生成机制,为后续基于扩散模型的模板重建方法提供数学工具与算法基础。

\subsection{前向扩散过程与反向去噪过程}

前向扩散过程将原始数据 $x_0$ 通过逐步添加高斯噪声的方式，在 $T$ 个时间步内转化为近似标准高斯分布的随机噪声 $x_T$，如图~\ref{fig:forward_diffusion}所示，图中直观展示了从清晰图像到纯高斯噪声的逐步退化过程。具体地，前向过程定义为一个马尔可夫链：
...
反向去噪过程旨在从随机噪声 $x_T\sim\mathcal{N}(0,I)$ 出发，通过逐步去噪恢复原始数据 $x_0$，如图~\ref{fig:reverse_diffusion}所示，图中直观展示了从纯高斯噪声逐步还原为清晰图像的生成过程。理想情况下，若已知真实的反向条件分布 $q(x_{t-1}|x_t)$，则可以精确地逆转扩散过程。然而，该分布依赖于整个数据集的全局信息，无法直接计算。
...
\subsection{采样策略与条件引导机制}

训练完成后，扩散模型的生成依赖于逐步去噪的采样过程。给定初始噪声 $x_T\sim\mathcal{N}(0,I)$，DDPM对时间步 $t=T,T-1,\ldots,1$ 依次迭代：将当前噪声状态 $x_t$ 输入噪声预测网络 $\epsilon_\theta(x_t,t)$ 以估计前向过程所添加的噪声，进而导出反向过程的均值估计：
\begin{equation}\label{eq:ddpm_mean}
      \mu_\theta(x_t,t) = \frac{1}{\sqrt{\alpha_t}}\left(x_t - \frac{1-\alpha_t}{\sqrt{1-\overline{\alpha}_t}}\epsilon_\theta(x_t,t)\right)
\end{equation}
在 $t>1$ 时叠加方差为 $\sigma_t^2$ 的高斯扰动以维持生成多样性，在最终步骤 $t=1$ 时则直接取均值以避免引入额外噪声：
\begin{equation}\label{eq:ddpm_sampling}
      x_{t-1} = \begin{cases} \mu_\theta(x_t,t) + \sigma_t\,z,\quad z\sim\mathcal{N}(0,I) & t > 1 \\ \mu_\theta(x_t,t) & t = 1 \end{cases}
\end{equation}
其中 $\sigma_t$ 通常取 $\sqrt{\beta_t}$ 或 $\sqrt{\tilde{\beta}_t}$。经 $T$ 步迭代后，所得 $x_0$ 即为最终生成结果。标准DDPM采样需完整执行 $T$（通常为1000）步，计算开销较大。为提高采样效率，去噪扩散隐式模型（Denoising Diffusion Implicit Models，DDIM）通过设置 $\sigma_t=0$ 构造确定性采样轨迹：
...
\end{equation}
DDIM允许使用任意子序列时间步采样，可将步数从1000步降低至50步甚至10步而无需重新训练，显著提升了采样效率。从连续时间视角看，前向扩散可建模为随机微分方程（Stochastic Differential Equation，SDE），对应的反向SDE通过学习得分函数 $\nabla_x\log p_t(x)$ 来生成样本，该视角与离散时间DDPM框架在理论上等价，并为DDIM等加速方法提供了统一的数学基础。

在实际应用中，往往需要根据特定条件控制生成内容，扩散模型的条件生成通过在采样过程中引入条件信息 $y$ 来实现，核心思想是学习条件分布 $p(x|y)$ 的得分函数 $\nabla_x\log p(x_t|y)$。若已有参数为 $\phi$ 的分类器 $p_\phi(y|x_t)$ 在噪声数据上预先训练完成，则根据贝叶斯规则可将条件得分分解为无条件得分与分类器梯度之和，通过引导强度 $\omega$ 调节条件信息权重：
\begin{equation}\label{eq:guided_score}
      \tilde{\nabla}_x\log p(x_t|y) = \nabla_x\log p(x_t) + \omega\nabla_x\log p_\phi(y|x_t)
\end{equation}
为避免训练额外分类器的代价，无分类器引导（Classifier-Free Guidance）在训练阶段以一定概率将条件 $y$ 替换为空标记 $\emptyset$，使模型同时学习条件分布与无条件分布，采样时通过线性外推组合两者的得分：
...
\end{equation}
其中 $\omega>1$ 可增强条件影响，但过大的 $\omega$ 可能导致生成样本偏离真实分布。对于本研究中的模板重建任务，条件信息为目标人脸嵌入 $f_{\text{target}}$，可将嵌入相似度定义为能量函数 $E(x;f_{\text{target}}) = -\lambda\cdot\text{sim}(F(x), f_{\text{target}})$，其中 $F(\cdot)$ 为预训练人脸识别网络，$\lambda$ 为引导强度。在采样过程中通过反向传播计算能量梯度并叠加到无条件得分上：
\begin{equation}\label{eq:energy_score}
      \tilde{\nabla}_x\log p(x_t|f_{\text{target}}) = \nabla_x\log p(x_t) - \nabla_x E(x_t;f_{\text{target}})
\end{equation}
这种基于能量的引导方式无需重新训练生成模型，可灵活引入身份一致性、感知质量等多种约束，为第3章中嵌入一致性约束的模板重建方法提供了直接路径。

\section{低秩适配微调}\label{sec:peft_lora}

在深度神经网络特别是大规模预训练模型的微调任务中，全参数微调往往面临计算资源消耗巨大、容易过拟合、存储与部署成本高昂等挑战。参数高效微调方法（Parameter-Efficient Fine-Tuning，PEFT）应运而生，旨在通过仅更新模型的少量参数或引入少量可训练模块，实现对下游任务的高效适配。低秩适配（Low-Rank Adaptation，LoRA）\cite{hu2022lora}作为PEFT方法中的代表性方法，已在自然语言处理、计算机视觉等领域展现出优异的性能与效率平衡，并被广泛应用于大型语言模型与扩散生成模型等的任务定制化微调中。

本节系统性地阐述低秩适配微调的理论基础、数学原理、工程实现策略及其在模板重建任务中的应用方法，为第3章中基于扩散模型的个性化模板重建提供方法支撑。
...
参数高效微调的理论基础源于神经网络参数空间的低秩结构与任务适配的内在维度假设\cite{aghajanyan2021intrinsic}。研究表明，尽管深度神经网络的参数空间维度极高，但针对特定任务的有效优化往往可以在远低于全参数空间维度的子空间中实现。具体地，Aghajanyan等人通过实验发现，对于给定的下游任务，存在一个低维参数子空间，在该子空间中进行优化即可达到与全参数微调相当的性能。形式化地，假设任务适配所需的参数更新 $\Delta\theta$ 位于参数空间的一个低秩子空间中，即$\Delta\theta \approx V\phi$，其中 $V\in\mathbb{R}^{d\times r}$ 为子空间的基向量矩阵，$\phi\in\mathbb{R}^r$ 为低维表示，$r\ll d$。这一假设为低秩适配方法提供了理论支撑：通过显式地将参数更新约束在低秩结构上，可以在大幅降低可训练参数量的同时，保持模型的适配能力。
...
\subsection{模块适配与权重合并}

在计算机视觉的生成模型中，LoRA通常应用于以下关键模块：（1）注意力机制的投影矩阵，Transformer架构或U-Net中的自注意力与交叉注意力模块包含Query、Key、Value与输出投影矩阵（$W_Q, W_K, W_V, W_O$），在这些投影层上应用LoRA能够高效地调整注意力模式，使模型关注任务特定的特征区域；（2）前馈网络（FFN），FFN中的线性层在特征变换与非线性映射中起关键作用，对FFN的权重矩阵应用LoRA可以调整特征的表示空间，适配新的数据分布或任务需求；（3）卷积层的点卷积（$1\times1$ Convolution），可以通过低秩分解调整通道间的特征融合权重；（4）条件注入层，在条件生成任务中通过AdaLN、交叉注意力等机制注入条件信息，应用LoRA能够精细控制条件对生成结果的影响。实践中，通常根据任务需求与计算资源约束选择性地在若干关键层上引入低秩适配。

在部署阶段，训练完成后可将LoRA的低秩增量 $\Delta W = \frac{\alpha}{r}BA$ 与原始权重 $W_0$ 合并为单一权重矩阵：
\begin{equation}\label{eq:lora_merge}
      W_{\text{merged}} = W_0 + \frac{\alpha}{r}BA
\end{equation}

合并后的模型在推理时与标准模型无异，无需额外的矩阵乘法操作。LoRA的重要优势在于支持多任务模块化部署，例如在多目标模板重建场景中，可采用"一个基础模型+多组LoRA权重"的架构：（1）共享基础模型：所有任务或目标共享同一预训练的基础模型 $\mathcal{M}_{\theta_0}$，该模型保持冻结状态；（2）任务特定LoRA：为每个任务或目标训练一组独立的LoRA权重 $\{A_i, B_i\}$，存储开销仅为基础模型参数量的 $\frac{r(d_{\text{in}}+d_{\text{out}})}{d_{\text{out}}\times d_{\text{in}}}$ 倍；（3）动态加载：在推理时根据任务需求动态加载对应的LoRA权重，实现快速切换。该架构在存储成本、部署灵活性与推理效率上均具有显著优势，特别适用于需要支持大量个性化配置的实际应用场景。

\section{本章小结}

本章系统构建了面向生物特征识别模型图像重建方法的理论基础，为后续章节的方法设计与实验评估提供了必要的方法支撑。

第~\ref{sec:face_recognition}~节建立了深度人脸识别模型的形式化框架，阐述了特征提取网络、嵌入归一化与相似度度量等核心组件，分析了度量学习方法在嵌入空间中构造判别性表示的机制，为定义图像重建方法的目标函数提供了形式化基础。第~\ref{sec:diffusion_models}~节系统推导了扩散概率模型的数学原理，从前向扩散的马尔可夫链到反向去噪的变分下界优化，阐述了后验分布、重参数化技巧、噪声预测目标与得分函数估计的内在联系，介绍了条件生成机制与DDIM等高效采样策略。第~\ref{sec:peft_lora}~节阐述了低秩适配微调的理论与实践，分析了LoRA在参数效率、计算复杂度与存储开销方面的优势，介绍了权重合并与模块化部署等工程策略。

本章在人脸识别的嵌入表示、扩散模型的条件生成、低秩适配的参数高效微调三个层面形成了完整的方法体系，为后续各章节图像重建方法的设计与实验评估奠定了坚实基础。
