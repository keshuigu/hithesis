...
生物特征识别系统已广泛应用于身份验证、访问控制、金融支付等关键领域，通常将用户的生物特征转换为固定维度的嵌入向量（模板），并通过相似度计算实现身份匹配。与传统认证机制相比，生物特征认证具有便捷性与难以伪造性等优势，但随着深度学习技术的深入应用，模板泄露导致的隐私风险显著增加：模板具有不可撤销性，且随着生成对抗网络与扩散模型技术的发展，从泄露模板逆向重建原始图像已成为现实威胁，攻击者可利用重建图像进行身份伪造或隐私侵犯。

本章为后续提出的模板逆向攻击方法与防御机制建立理论基础，系统阐述四个核心支柱：\textbf{（1）人脸识别系统}——详述系统架构、训练范式（对比损失、三元组损失、角度边距损失）、评估协议及模板安全性问题，为定义攻击目标函数与评估指标提供概念框架；\textbf{（2）扩散概率模型}——推导前向扩散、反向去噪、变分下界优化与条件生成机制（分类器引导、无分类器引导、基于能量的引导），并介绍潜在扩散模型与高效采样器，为构建模板重建方法提供数学工具；\textbf{（3）换脸生成技术}——回顾基于3DMM、自编码器、GAN与扩散模型的技术路线，分析身份-属性解耦、感知质量优化等共性问题，为损失函数设计与网络架构选择提供方法论参考；\textbf{（4）参数高效微调}——阐述低秩适配（LoRA）的理论与工程实践，包括应用策略、超参数调优与推理优化，为大规模模板重建实验提供可行路径。

上述四个支柱形成完整闭环：人脸识别系统定义攻击目标与评估标准，扩散模型提供生成框架，换脸技术提供方法论基准，参数高效微调保障工程实现。本章组织结构如下：第2.2节阐述人脸识别模型，第2.3节推导扩散模型原理，第2.4节回顾换脸技术，第2.5节介绍参数高效微调，第2.6节总结全章并阐明各模块在后续研究中的应用。
