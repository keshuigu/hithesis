...
模板逆向重建攻击针对基于特征提取的检索型人脸识别系统。设特征提取器为 $F: \mathbb{R}^{H \times W \times C} \to \mathbb{R}^d$，将输入图像 $x$ 映射为 $d$ 维特征嵌入 $e = F(x)$。在实际部署的人脸识别系统中，为提高检索效率，通常仅存储用户的低维特征模板而非原始图像。攻击者通过数据库泄露、内部威胁或侧信道攻击等途径获得目标身份的特征模板 $t = F(x_0) \in \mathbb{R}^d$，其目标是构造逆映射函数 $g: \mathbb{R}^d \to \mathbb{R}^{H \times W \times C}$，生成图像 $\hat{x} = g(t)$，使得其满足两个核心特性。

首先是身份一致性，即生成图像的特征嵌入应与目标模板在度量空间中高度接近。形式化地，要求满足 $F(\hat{x}) \approx t$，具体判据为相似度满足 $\text{sim}(F(\hat{x}), t) > \tau$，其中 $\text{sim}(\cdot, \cdot)$ 为相似度度量函数，在基于余弦距离的人脸识别系统中通常采用余弦相似度 $\text{sim}(a, b) = \frac{a \cdot b}{\|a\|\|b\|}$，$\tau$ 为识别系统的验证阈值，一般取值在0.3到0.6之间。其次是视觉真实性，即生成图像 $\hat{x}$ 应具有自然的人脸特征和纹理细节，在感知质量上与真实人脸图像无明显差异，需要属于自然人脸图像流形 $\mathcal{M}_{\text{natural}}$。
...
（1）几何对齐性：与ArcFace等人脸识别系统的单位超球面设计保持一致，确保特征空间的优化方向与识别器的决策边界对齐。（2）判别性增强：通过引入负样本对比，显式增大生成特征与非目标类别特征的角度距离，提升特征的类别区分度。负样本通过随机采样获得，确保对比约束的多样性与计算效率的平衡。（3）梯度稳定性：相比欧氏距离，余弦相似度在特征高度相似时仍能提供有效的梯度信号，避免梯度消失问题。
