...
python finetune_lora.py \
  ...
  --data /path/to/faces \
  --out_dir /exp/edm_tia_finetune \
  --r 16 --alpha 16 --lr 1e-4 --steps 1000
\end{verbatim}
...
\section{可复现的目录布局与示例命令}
为便于他人复现与复查实验结果，建议在项目中按以下约定组织文件：
\begin{itemize}
  \item configs/: YAML/JSON 配置文件（训练/微调/推理）
  \item data/: 数据下载与预处理脚本（说明许可证与来源）
  \item checkpoints/: 基线与微调检查点（含 LoRA 权重）
  \item experiments/: 每次实验的日志、度量与可视化输出
  \item scripts/: 常用训练/推理脚本（train_edm.py, finetune_lora.py, infer.py）
\end{itemize}

示例命令（可直接复制运行，需替换路径）：
\begin{verbatim}
# 基础训练（若使用预训练则跳过）
python scripts/train_edm.py --config configs/edm_base.yaml --work-dir experiments/base

# LoRA 微调
python scripts/finetune_lora.py --ckpt checkpoints/edm_base.pt --templates data/templates.npy \
  --out experiments/lora_r16 --r 16 --alpha 16 --lr 1e-4 --steps 1000

# 推理
python scripts/infer.py --ckpt experiments/lora_r16/checkpoint.pt --template data/target_tpl.npy \
  --out experiments/lora_r16/out --steps 50 --cfg_scale 2.0 --proj_steps 1 --proj_eta 5e-3
\end{verbatim}

\section{实验设计与消融计划}
为系统评估方法的鲁棒性与普适性，建议至少包含以下对比与消融实验：
\begin{itemize}
  \item 基线对比：无条件 EDM（或 LDM）采样、CFG、能量引导、LoRA 微调四种主要策略的横向对比；
  \item 消融维度：guidance scale、LoRA 秩 $r$、LoRA 是否与 attention head 结合、采样步数、投影步 $K_{proj}$ 的影响；
  \item 数据维度：在不同数据偏差下（种族/年龄/光照）评估攻击成功率与失败模式；
  \item 查询预算敏感性：在黑盒-可查询场景下评估随着查询次数 $Q$ 增加的性能曲线；
  \item 防御有效性测试：评估模板扰动（量化/签名/噪声注入）、差分隐私训练对攻击成功率的降低效果。
\end{itemize}

每个实验应至少报告：平均余弦相似度、TAR@FAR、top-1/5、FID/LPIPS、平均采样步数与平均单样本耗时。并提供成功/失败样本的可视化集合。

\section{伦理、合规与防御建议}
在开展模板逆向研究时，必须严格遵守伦理与法律约束：
\begin{itemize}
  \item 数据使用合规：仅使用具有明确许可的公开数据集或已获得被试同意的数据；在论文中列明数据来源与许可条款；
  \item 受试者隐私保护：对发布的示例图像进行脱敏或仅展示合成图像的低分辨率缩略图；避免发布能直接识别真实个人的高分辨率重建；
  \item 风险披露：在论文与代码库中明确讨论潜在滥用风险并提供缓解建议（模板加密、签名化模板、差分隐私训练）；
  \item 防御建议：建议服务端对模板存储采用签名化/哈希检测、对外部查询设置严格速率限制与认证、并考虑模板变换或随机化以降低可逆性。
\end{itemize}
