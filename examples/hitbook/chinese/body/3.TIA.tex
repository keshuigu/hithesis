% !Mode:: "TeX:UTF-8"
\chapter{基于角度约束对比学习的模板重建人脸方法}[Face Image Inverse Reconstruction from Feature Templates via Angular Constraint Contrastive Learning]\label{chap:TIA}
\section{引言}[Introduction]

本章针对基于模板匹配的人脸识别模型，研究将已泄露的特征模板逆向重建为可感知人脸图像的技术方法。现有方法主要面临两方面挑战：一是传统基于优化的方法容易陷入局部最优且生成质量有限，引入生成式先验虽可改善视觉效果，但在特征空间的优化缺乏针对性设计，难以充分利用人脸识别模型的几何结构特性；二是多目标优化中像素与特征损失的权重平衡缺乏系统解决方案。
...
本章首先形式化定义模板逆向攻击的问题与攻击成功判据，随后详细阐述基于扩散模型的重建架构、角度约束对比学习损失设计、任务不确定性加权框架、渐进式训练策略与模板条件梯度引导推理机制。基于上述方法，本章在MOBIO、LFW、AgeDB和IJB-C等标准数据集上使用ArcFace模型进行系统实验，通过与现有代表性方法的对比验证ArcTIA在攻击成功率和生成质量上的优势，并通过消融实验量化各关键设计模块的性能贡献。

\section{形式化问题定义}[Problem Formulation]

模板逆向重建攻击针对基于特征提取的检索型人脸识别模型。设特征提取器为 $F: \mathbb{R}^{H \times W \times C} \to \mathbb{R}^d$，将输入图像 $x$ 映射为 $d$ 维特征嵌入 $e = F(x)$。在实际部署的人脸识别模型中，为提高检索效率，通常仅存储用户的低维特征模板而非原始图像。攻击者通过数据库泄露、内部威胁或侧信道攻击等途径获得目标身份的特征模板 $t = F(x_0) \in \mathbb{R}^d$，其目标是构造逆映射函数 $g: \mathbb{R}^d \to \mathbb{R}^{H \times W \times C}$，生成图像 $\hat{x} = g(t)$，使得其满足两个核心特性。

首先是身份一致性，即生成图像的特征嵌入应与目标模板在度量空间中高度接近。形式化地，要求满足 $F(\hat{x}) \approx t$，具体判据为相似度满足 $\text{sim}(F(\hat{x}), t) > \tau$，其中 $\text{sim}(\cdot, \cdot)$ 为相似度度量函数，在基于余弦距离的人脸识别模型中通常采用余弦相似度 $\text{sim}(a, b) = \frac{a \cdot b}{\|a\|\|b\|}$，$\tau$ 为识别模型的验证阈值，一般取值在0.3到0.6之间。其次是视觉真实性，即生成图像 $\hat{x}$ 应具有自然的人脸特征和纹理细节，在感知质量上与真实人脸图像无明显差异，可形式化为要求 $\hat{x}$ 属于自然人脸图像流形 $\mathcal{M}_{\text{natural}}$。

形式化地，模板逆向重建攻击问题可建模为如下约束优化问题：
...
其中第一项 $\mathcal{L}_{embed}$ 为嵌入空间距离损失，度量生成图像特征与目标模板的偏差，可采用余弦距离 $1-\cos(F(x),t)$ 或欧氏距离 $\|F(x)-t\|_2^2$；第二项 $\mathcal{L}_{perc}$ 为感知质量损失，约束生成图像的视觉真实性，常采用预训练VGG网络的多层特征距离或学习的感知损失LPIPS；第三项 $R(\cdot)$ 为正则化项或先验约束，用于防止过拟合并引导优化过程朝向自然图像流形；$\lambda_{perc}$ 和 $\lambda_{reg}$ 为权衡系数，控制各目标之间的相对重要性。

式\eqref{eq:tia_optimization}中的约束条件 $\hat{x} \in \mathcal{M}_{\text{natural}}$ 表示生成样本必须位于自然人脸图像流形上，这一约束在模板逆向攻击中具有双重意义。首先从攻击有效性角度，人脸识别模型的特征提取器在训练时假设输入服从自然人脸分布，其学习到的特征空间几何结构和判别边界均基于此假设。若生成图像显著偏离 $\mathcal{M}_{\text{natural}}$，其特征表示可能落入特征空间的低置信区域甚至分布外区域，导致即使优化目标函数收敛，生成图像的特征与目标模板仍可能存在语义层面的不一致，从而降低攻击成功率。其次从隐蔽性角度，模板逆向攻击的实际威胁在于攻击者能够重建出视觉上可信的人脸图像，用于伪造身份、社会工程等恶意用途。若生成样本包含明显的伪影、非自然纹理或不合理的面部结构，将立即被人类观察者或异常检测系统识别，使攻击失去实际价值。因此，$\mathcal{M}_{\text{natural}}$ 约束不仅是提升攻击成功率的技术必要性，更是保证攻击隐蔽性和实用性的关键条件。在实际优化中，该约束通过感知损失 $\mathcal{L}_{perc}$ 和正则化项 $R(\cdot)$ 隐式实现，而本章采用的扩散模型生成先验则在模型层面天然保证生成样本位于自然图像流形附近。

攻击成功判据包括三个层次：强成功是指生成图像能够直接通过身份验证系统，即 $\text{sim}(F(\hat{x}), t) \geq \tau$；弱成功是指虽未达到验证阈值但相似度显著高于随机基线，表明攻击部分泄露了目标身份的生物特征信息；视觉质量成功是指重构图像具有合理的人脸特征与自然外观，能够通过人类视觉检验。
...
\section{基于角度约束对比学习的模板逆向重建算法}[Diffusion-based Reconstruction Architecture and Angular Constraint Loss Design]
\label{sec:tia_architecture}

\subsection{整体结构设计}

ArcTIA的整体架构由骨干生成模型与双分支协同结构两部分构成。本节首先阐述骨干生成模型的选择依据，随后介绍完整的网络架构设计。

\subsubsection{骨干生成模型的选择}

本章选择EDM\cite{karras2022elucidating}作为骨干生成模型。EDM通过系统性分析扩散模型的设计空间，在噪声调度、网络预处理与采样策略等多个方面提出了改进方案。相较于传统DDPM与DDIM，EDM具有以下显著优势：（1）通过重新参数化去噪网络的输入与输出，消除了训练过程中的数值不稳定性；（2）提出了更优的噪声调度方案，确保在不同噪声水平下模型预测误差分布更加均衡；（3）通过统一的采样器设计，在保持生成质量的同时，显著降低了所需的函数评估次数；（4）为条件引导提供了更加稳健的梯度传播路径，使得在高引导强度下仍能保持生成样本的多样性与真实性。

具体而言，EDM将第~\ref{chap:theory}章中DDPM的噪声预测目标（式~\eqref{eq:noise_loss}）重新参数化为去噪目标。DDPM训练网络预测噪声$\epsilon$，而EDM直接预测清晰图像$x_0$，其训练损失为：
...
其中$c_{\text{skip}}(t)$、$c_{\text{out}}(t)$、$c_{\text{in}}(t)$和$c_{\text{noise}}(t)$为基于噪声水平$t$的预处理系数函数，$F_\theta$为核心神经网络。该预处理设计确保了网络输入与输出的数值范围在训练过程中保持稳定，显著改善了梯度传播特性与训练收敛速度。综合以上优势，EDM在人脸生成等高分辨率图像合成任务中，相比DDPM可实现更快的收敛速度、更低的训练资源消耗以及更高的样本质量，因此本章选择EDM作为模板逆向重建方法的骨干生成模型。
...
该两阶段策略结合了固定阶段划分的稳定性与任务不确定性参数的自适应性，既避免了训练初期多任务优化的不稳定，又充分利用了任务不确定性加权机制的理论优势。

\subsubsection{完整训练算法}

整合前述两阶段策略，完整的训练流程如算法~\ref{alg:edm_tia_train}所示，图~\ref{fig:edm_tia_train}展示了训练流程的整体架构。

训练的数据流如下：每次迭代从训练集中随机采样一批图像 $\{(x_0^{(j)}, y^{(j)})\}$，依据EDM的噪声调度 $p_\sigma$ 采样噪声水平 $\sigma^{(j)}$，并添加标准高斯噪声构造带噪输入 $x_t^{(j)}$；去噪网络 $f_\theta$ 以带噪图像、身份标签与噪声水平为条件预测清晰图像 $\hat{x}_0^{(j)}$；预测结果随即通过冻结的特征提取器 $F$ 映射至特征空间，供像素损失 $\mathcal{L}_{\text{pixel}}$、角度约束特征损失 $\mathcal{L}_{\text{feat}}$ 与多样性损失 $\mathcal{L}_{\text{div}}$ 的计算使用。

梯度更新策略依训练阶段而异。预热阶段（$t < t_{\text{warmup}}$）仅对像素损失 $\mathcal{L}_{\text{pixel}}$ 求梯度并更新网络参数 $\theta$，任务不确定性参数保持冻结；主训练阶段（$t \geq t_{\text{warmup}}$）启用完整的式~\eqref{eq:tia_total_loss}混合损失，网络参数 $\theta$ 与任务不确定性参数 $\sigma_p, \sigma_f$ 联合优化。随着训练推进，$\sigma_p$ 与 $\sigma_f$ 根据各任务的优化难度自动调整权重：当某一损失项收敛困难时，对应的 $\sigma$ 自动增大以降低其权重，避免其主导梯度方向；反之则保持较小值以维持足够的监督强度。这一机制使模型在不同训练阶段均能在视觉质量与特征匹配精度之间保持合理的权衡，而无需人工搜索权重超参数。

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{images/train_tia.pdf}
  \vspace{-1em}
...
引导项的引入可从分类器引导扩散模型~\cite{rombach2022high}的理论框架得到严格的数学解释。在标准扩散采样过程中，去噪步骤依据无条件分布$p_\theta(x_{i-1}|x_i)$进行；当引入模板匹配条件$c = \{F(x) \approx t\}$后，目标分布变为条件分布$p_\theta(x_{i-1}|x_i, c)$。由贝叶斯定理，该条件分布可分解为：
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{images/infer_tia.pdf}
  \vspace{-3em}
...
本节通过系统实验验证前述提出的基于角度约束对比学习的模板重建人脸方法的有效性。实验在标准测试集上与现有代表性方法进行定量对比以评估攻击成功率与生成质量，并通过消融实验量化各关键模块的性能贡献。
...
表~\ref{tab:tia_sota_comparison}展示了ArcTIA与现有模板逆向攻击方法在四个基准数据集上的性能对比。
...
表~\ref{tab:tia_sota_comparison}展示ArcTIA与现有模板逆向攻击方法在四个基准数据集上的攻击成功率对比。在FMR=$10^{-2}$的常规安全场景下，ArcTIA在MOBIO、LFW、AgeDB和IJB-C上分别达到97.38\%、94.23\%、75.64\%和82.91\%，四项指标均超越次优方法Shahreza et al.，平均攻击成功率达到87.54\%，较次优方法的85.38\%提升2.16个百分点。在FMR=$10^{-3}$的高安全场景下，ArcTIA在MOBIO、AgeDB和IJB-C上分别取得87.87\%、65.81\%和55.58\%，平均攻击成功率达到73.41\%，较次优方法的68.95\%提升4.46个百分点。
...
本章针对人脸识别模型的模板逆向重建任务，提出基于角度约束对比学习与自适应加权的模板重建人脸方法（ArcTIA）。通过将特征优化目标与识别模型的超球面决策边界对齐，结合任务不确定性加权框架自动平衡多目标损失，以及在推理阶段引入模板条件梯度引导机制，ArcTIA在保持生成质量的同时显著提升了攻击成功率。实验结果表明，ArcTIA在MOBIO、LFW、AgeDB、IJB-C等标准数据集的综合指标上超越现有方法，消融实验验证了各核心模块的有效性与协同作用。